...
//				System.out.println("getItemByStringId 1013");
//			} else {
//				c = ConnectionPool.getConnection(alias);
//				System.out.println("getItemByStringId 1016");
//			}
			c = ConnectionPool.getConnection(un,pw,alias);
	...
	public static  ClientA getClientById(String un,String pw,String alias, String branch, String itemId) {
		ClientA customer = new ClientA();
		System.out.println("param branch="+branch+"alias="+alias+" itemid="+itemId+" un="+un+" pw="+pw);
		ISLogger.getLogger().error("param branch="+branch+"alias="+alias+" itemid="+itemId+" un="+un+" pw="+pw);
		PreparedStatement ps = null;
		CallableStatement cs = null;
		ResultSet rs = null;
		Connection c = null;
		try {
			...
			cs = c.prepareCall("{call info.init()}");
			cs.execute();
			ps = c.prepareStatement("SELECT * FROM v_client_humo WHERE branch = ? and id_client=?");
			ps.setString(1, branch);
			ps.setString(2, itemId);
			rs = ps.executeQuery();
			if (rs.next()) {
