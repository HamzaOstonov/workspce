...
public class TransactionService
{
    ...
    throws Exception
  {
    ...
    ISLogger.getLogger().info("TrService line 44");
    ...
    if (current_templates != null) {
      for (int i = 0; i < current_templates.size(); i++)
      {
        ...
        ISLogger.getLogger().info("TrService line 57");
        Bf_tr_acc tr_acc_ct = 
          (Bf_tr_acc)((HashMap)this.tr_accs.get((String)parameters.get("branch"))).get(Long.valueOf(tr_template.getAcc_ct()));
        
        Bf_tr_acc tr_acc_dt = 
          (Bf_tr_acc)((HashMap)this.tr_accs.get((String)parameters.get("branch"))).get(Long.valueOf(tr_template.getAcc_dt()));
        ...
        if (tr_acc_ct.getAccount().matches("[0-9]+")) {
          doc_params.put("ACC_CO", tr_acc_ct.getAccount());
        	...
        	name_parama = tr_acc_ct.getAccount();
        	doc_params.put("ACC_CO", parameters.get(tr_acc_ct.getAccount()));
        }
        ISLogger.getLogger().info("TrService line 77");
        
        name_parama += " acc_dt id = "+tr_acc_dt.getId() + " oper_id = "+operation_id + " boolCL = "+tr_acc_dt.getAccount();
        if (tr_acc_dt.getAccount().matches("[0-9]+")) {
          doc_params.put("ACC_CL", tr_acc_dt.getAccount());
        	...
        	doc_params.put("ACC_CL", parameters.get(tr_acc_dt.getAccount()));
        }
        ...
        System.out.println("Service.get_day(c):"+Service.get_day(c));
        
        //doc_params.put("BANK_CL", ((String)doc_params.get("ACC_CL")).substring(12, 17));
        
        HashMap<String, String> bank_filials = Service.get_bank_filials(c);
        
        ISLogger.getLogger().info("TrService line 105");
        ...
        ISLogger.getLogger().info("TrService line 120");
        
        
        doc_params.put("BANK_CL", tr_acc_dt.getAcc_mfo());
        name_parama += " ACC_CL = "+doc_params.get("ACC_CL")+" ACC+MFO = "+tr_acc_dt.getAcc_mfo();
        ...
        ISLogger.getLogger().info("TrService line 139");
        ...
        doc_params.put("id_transh", Long.valueOf(tr_template.getId_transh_purp()));
        ...
        doc_params.put("id_transh_purp", Long.toString(tr_template.getId_transh_purp()));
        doc_params.put("cs_prep", parameters.get("cs_prep"));
        
        ISLogger.getLogger().info("TrService line 178");
    ...
    throws Exception
  {
    ...
    System.out.println("init tr_templates " + tr_templates.size());
    this.tr_accs = Service.get_accounts(c);
    System.out.println("init tr_accs " + tr_accs.size());
    this.bf_tr_paytypes = Service.get_bf_tr_paytypes(c);
    System.out.println("init bf_tr_paytypes " + bf_tr_paytypes.size());
  }
}
