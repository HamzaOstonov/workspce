...
public class FileService
{
    ...
    public static List<HumoFileRecords> getFileRecords(final long empc_file_id, final long fileType) {
        ...
        Label_0449: {
            try {
                if (fileType == 1L) { /*1- b fayl, 2- expt fayl, 4- A fayl, 5- i fayl*/
                    c = ConnectionPool.getConnection();
                    final PreparedStatement ps = c.prepareStatement("SELECT r.id,r.state_id,r.term_nr,r.merchant,r.abvr_name,r.card,r.mcc_code,r.tran_type,r.amount,r.batch_date,"+
                    		"(select e.text from EMPC_B_RECORDS_PRC_ERRORS e where r.id = e.record_id) errText,"+
                    		"s.name FROM empc_files_b_transactions r,empc_expt_records_states s "+
                    		"WHERE r.empc_file_id = ? and r.state_id=s.id order by state_id desc");
                    ps.setLong(1, empc_file_id);
                    final ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
                    ...
                    c = ConnectionPool.getConnection();
                    final PreparedStatement ps = c.prepareStatement("SELECT r.id, r.state_id, r.mcc_code, r.merchant,r.abvr_name,r.card,r.tranz_acct, r.term_id,r.tran_type,"+
                    		"r.tran_amt,r.tran_date_time,(select e.text from EMPC_EXPT_RECORDS_PRC_ERRORS e where r.id = e.record_id) errText,s.name,"+
                    		"r.tran_amt2 FROM empc_expt_records r,empc_expt_records_states s WHERE r.empc_file_id = ? and r.state_id=s.id order by state_id desc");
                    ps.setLong(1, empc_file_id);
                    final ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
                    ...
                    c = ConnectionPool.getConnection();
                    final PreparedStatement ps = c.prepareStatement("SELECT r.id, r.state_id, r.mcc mcc_code, r.merchant_id merchant, "+
                                            "r.merchant_name abvr_name, r.pan card, '' tranz_acct, '' term_id, r.tran_type, "+
                                            "r.SETTL_AMNT tran_amt,r.tran_date_time,(select e.text from EMPC_a_RECORDS_PRC_ERRORS e where r.id = e.record_id) errText,s.name, "+
                                            "r.FEE_AMNT tran_amt2 FROM empc_a_records r,empc_expt_records_states s  "+
                                            "WHERE r.empc_file_id = ? and r.state_id=s.id order by state_id desc");
                    ps.setLong(1, empc_file_id);
                    final ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
                    ...
                    final PreparedStatement ps = c.prepareStatement("SELECT r.id, r.state_id, r.mcc mcc_code, r.merchant_id merchant, "+
                                            "r.merchant_name abvr_name, r.pan card, '' tranz_acct, '' term_id, r.tran_type, "+
                                            "r.SETTL_AMNT tran_amt,r.tran_date_time,(select e.text from EMPC_i_RECORDS_PRC_ERRORS e where r.id = e.record_id) errText,s.name, "+
                                            "r.FEE_AMNT tran_amt2 FROM empc_i_records r,empc_expt_records_states s  "+
                                            "WHERE r.empc_file_id = ? and r.state_id=s.id order by state_id desc");
                    ps.setLong(1, empc_file_id);
                    final ResultSet rs = ps.executeQuery();
                    while (rs.next()) {
