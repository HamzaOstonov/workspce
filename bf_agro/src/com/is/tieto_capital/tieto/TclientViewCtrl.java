...
public class TclientViewCtrl extends GenericForwardComposer
{
	...
	private com.is.tieto_capital.customer.PagingListModel CustPagingListModel;
	...
	private capitalBank.IssuingWS.IssuingPortProxy pp = null;
	...
	public void onSelect$dataGrid()
	{
		if (current != null)
		{
			...
			List<CardInfo> cardInfos = TclientService.getAccInfo(current,alias, issuingPortProxy , smart).get(0).getCardlist();
	...
	public void onDoubleClick$accGrid()
	{
		...
		if (res.getCode() == 0)
		{
			...
			if (current.getClient() != null) 
				cardInfos = TclientService.getAccInfo(current,alias, issuingPortProxy , smart).get(0).getCardlist();
			accGrid.setModel(new BindingListModelList(cardInfos , false));
			return;
		}
		...
		paywnd$payGrid.setModel(new ListModelList(TclientService.getBf_tieto_trans(selectedCard.getBank_account(), branch, alias)));
		paywnd$list_amnt.setSelectedIndex(-1);
		paywnd$amnt.setValue(BigDecimal.valueOf(0));
		paywnd$list_amnt.setModel((new ListModelList(TclientService.getPayments4Tieto(branch, selectedCard.getBank_account(), alias))));
	...
	private void pay(int deal_id, int rep_type_id, int act_id)
	{
		...
		{
			alert(res.getName());
			accGrid.setModel(new BindingListModelList(TclientService.getAccInfo(current,alias, issuingPortProxy , smart), false));
			return;
		}
	...
	public void onClick$btn_pay$paywnd()
	{
		...
		try
		{
			...
			if (res.getCode() == 1)
			{
				...
				Bf_tieto_trans bf_tieto_trans = new Bf_tieto_trans();
				bf_tieto_trans.setEMPID(String.valueOf(uid));
				bf_tieto_trans.setGENID(generalInfo.getID());
				bf_tieto_trans.setTIETOID(TIETOID);
				bf_tieto_trans.setBRANCH(branch);
				bf_tieto_trans.setSTATE(STATE);
				
				res = TclientService.insertIntoBfTietoTrans(bf_tieto_trans);
				if (res.getCode() == 0)
				{
					alert(res.getName());
	...
	public void printp(TrPay trp, String report_file)
	{
		...
		params.put("ACCDOPER1", com.is.tieto_capital.tieto.TclientService.getkass_acc(branch, alias));
	...
	public void onClick$btn_add$addwnd()
	{
		...
		TietoCardSetting tcs = TclientService.getTietoCardSetting(addwnd$sproduct.getValue(), alias);
		TietoCustomer tc;
		tc = com.is.tieto_capital.customer.CustomerService.getTietoCustomer(current.getClient(), branch, alias);
	}
