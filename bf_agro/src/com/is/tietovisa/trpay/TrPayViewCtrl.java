...
public class TrPayViewCtrl extends GenericForwardComposer {
	...
	private IssuingPortProxy ipp = null;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		if (wayPaySourceContractNum == null || wayPaySourceContractNum == ""
			...
			wayPaySourceContractNum = ConnectionPool
					.getValue("OPENWAY_PAYSOURCE_CONTRACTNUM");
		...
		filter_box_oper.setModel((new ListModelList(TrPayService
				.getListOperActions(alias))));
		/*
		...
		/*
		 * pp = new IssuingPortProxy(ConnectionPool.getValue("TIETO_HOST",
		 * alias), ConnectionPool.getValue("TIETO_HOST_USERNAME", alias),
		...
		//"doAfterCompose TrPayViewCtrl 05");

		// onClick$btn_filter_act();
		refreshModel(_startPageNumber);
		//ISLogger.getLogger().error(
		//"doAfterCompose TrPayViewCtrl 06");
	...
	private void refreshModel(int activePage) {
		// if (access > 0) {
		//ISLogger.getLogger().error(		"refreshModel TrPayViewCtrl 01");
		...
		//ISLogger.getLogger().error(		"refreshModel TrPayViewCtrl 02");
		...
		ISLogger.getLogger().error(		"refreshModel TrPayViewCtrl 03");
		...
		try {
			...
			if (rs.getCode() < 0) {
				...
				if (TrPayService.isEndState(trpay1.getId(), alias, c)) {
					Res res = com.is.tietovisa.trpay.TrPayService.send2tieto(
							trpay1, alias);
					...
					Res res2 = com.is.tietovisa.trpay.TrPayService
	...
	public void action_tieto(Cur_cont cur_cont) {
		...
		try {
			...
			Res res = com.is.tietovisa.trpay.TrPayService.send2tieto(trpay1,
					alias);
	...
	public void onDoubleClick$dataGrid$grd() {
		...
		if (tbfrm.getSelectedIndex() == 1) {
			paytmpGrid.setModel(new BindingListModelList(TrPayService
					.getTrPayDocs(current.getId(), alias), false));
		}
	...
	private boolean send2t(TrPay tp) {
		...
		// RowType_ExecTransaction_Request parameters = new
		// RowType_ExecTransaction_Request();
		// OperationResponseInfoHolder responseInfo = new
		// OperationResponseInfoHolder();
		...
		// parameters.setTRAN_TYPE(Integer.toString(tp.getTieto_type()));//
		// tp.getOperation_id(),
		// // alias)));
		...
		// ipp.executeTransaction(connectionInfo, parameters, responseInfo,
		// details);
