...
public class AccPayService {
	...
	static {
		...
		AccPayService.msql2 = "visa_cards";
	}
	...
	public static int getCount(final CardIbs filter, final String alias) {
		Connection c = null;
	...
	private static List<FilterField> getFilterFields(final CardIbs filter) {
		...
		if (!CheckNull.isEmpty(filter.getCard())) {
			flfields.add(new FilterField(String.valueOf(getCond(flfields))
					+ "card=?", (Object) filter.getCard()));
		}
		if (!CheckNull.isEmpty(filter.getReal_card())) {
			flfields.add(new FilterField(String.valueOf(getCond(flfields))
					+ "real_card=?", (Object) filter.getReal_card()));
		}
	...
	public static List<CardIbs> getCustomersFl(final int pageIndex, final int pageSize, final CardIbs filter, final String alias) {
		System.out.println("1");
		final List<CardIbs> list = new ArrayList<CardIbs>();
		Connection c = null;
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
                final CardIbs customer = new CardIbs();
                
                //customer.setId(rs.getLong("id"));
                customer.setBranch(rs.getString("branch"));
                customer.setCard_name( rs.getString("card_name"));
                customer.setClient_id( rs.getString("client_id"));
                customer.setClient_b( rs.getString("client_b"));
                customer.setCard( rs.getString("card"));
                customer.setReal_card( rs.getString("real_card"));
                customer.setTranz_acct( rs.getString("tranz_acct"));
                customer.setCard_acct( rs.getString("card_acct"));
	...
	public static List<RefData> getMfo(final String branch) {
		return (List<RefData>) com.is.utils.RefDataService
				.getRefData(
	...
	public static String getUser_name2(int user_id, String branch, String alias)
    {
            String res = null;
            Connection c = null;
        ...
        try {
                ...
                PreparedStatement ps = c.prepareStatement("select full_name from v_bf_bank_users where id = ? and branch = ?");
                ps.setString(1, user_id+"");
                ps.setString(2, branch);
                ResultSet rs = ps.executeQuery();
                //rs.next();
	...
	public static String getUser_Title(int user_id, String branch, String alias)
    {
            String res = null;
            Connection c = null;
        ...
        try {
                ...
                PreparedStatement ps = c.prepareStatement("select title from v_bf_bank_users where id = ? and branch = ?");
                ps.setString(1, user_id+"");
                ps.setString(2, branch);
                ResultSet rs = ps.executeQuery();
                //rs.next();
