...
public class AddCstViewCtrl extends GenericForwardComposer {
	...
	private static HashMap<String, String> condCardSets = null;
	...
	private Listheader lh_base_supp, lh_client_id, lh_agreement_key, lh_account_no, lh_card_acct, lh_status2, lh_expiry2, lh_cl_role, lh_bank_c, lh_groupc, lh_ico_export;
	...
	public void doAfterCompose(final Component comp) throws Exception,
		...
		condCardSets = com.is.tietovisa.customer.CustomerService.getMapCardCondSets(alias);

		// filter.setP_pinfl("56789012340078");
		// filter.setId_client("60000001");
		filter.setEndpoint(listCustomerEndpoint);
		lh_base_supp.setVisible(false);
		lh_client_id.setVisible(false);
		lh_agreement_key.setVisible(false);
		lh_account_no.setVisible(false);
		lh_card_acct.setVisible(false);
		lh_status2.setVisible(false);
		lh_expiry2.setVisible(false);
		lh_cl_role.setVisible(false);
		lh_bank_c.setVisible(false);
		lh_groupc.setVisible(false);
		lh_ico_export.setVisible(false);
		...
		cardGrid.setItemRenderer(new ListitemRenderer() {
			...
			public void render(Listitem row, Object data) throws Exception {
				...
				//2024.12.17 shuerga balansni qushishimiz kerak
				//balans
				List<AccountBalance> balanceList=
				 com.is.tietovisa.customer.CustomerService
				.getAccountBalance_tieto(
						pAccInfo.getCARD(), new java.util.Date(), new java.util.Date(), "",
						CustomerService.mapConst
								.get(Cons.url_accountbalance));
				//(new BigDecimal(balanceList.get(0).getEND_BAL()).divide(new BigDecimal("100")).toString())))
				if (balanceList!=null && balanceList.size()>=0  )
				{
					row.appendChild(new Listcell((new BigDecimal(balanceList.get(0).getEND_BAL()).divide(new BigDecimal("100")).toString())));
				}
				else
				{
					row.appendChild(new Listcell("Error getting balance..."));
				}
				...
				//row.appendChild(new Listcell(pAccInfo.getCOND_SET() ));
				row.appendChild(new Listcell( condCardSets.get(pAccInfo.getCOND_SET())));
