...
public class CustomerService {
	...
	private static HashMap<String, String> mapCardCondSets;
	...
	public static HashMap<String, String> getMapCardCondSets(String alias) {
		if ( listCardCondSets == null || listCardCondSets.size() == 0)
			listCardCondSets = (List<RefData>) Utils
					.getRefData(
							"select cond_set data, cond_set||'-'||ccy||'-'||name label, 0 ord from bf_izd_cond_card order by cond_set",
		...
		if (mapCardCondSets==null) {
			mapCardCondSets=new HashMap<String, String>();
			for (int i = 0; i < listCardCondSets.size(); i++) {
				mapCardCondSets.put(listCardCondSets.get(i).getData(),
						listCardCondSets.get(i).getLabel());
			}
		}
		return mapCardCondSets;
	}
