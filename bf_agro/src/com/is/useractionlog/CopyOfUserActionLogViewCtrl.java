...
private void refreshModel(int activePage){
    ...
    model1 = new PagingListModel(activePage, _pageSize, filter, alias );
    _totalSize = model1.getTotalSize(filter, alias);

    useractionlogPaging.setTotalSize(_totalSize);

    dataGrid.setModel((ListModel) model1);
    if (model1.getSize()>0){
...
public void onClick$btn_last() {
        dataGrid.setSelectedIndex(model1.getSize()-1);
        sendSelEvt();
}
...
public void onClick$btn_next() {
        if (dataGrid.getSelectedIndex()!=(model1.getSize()-1)){
        dataGrid.setSelectedIndex(dataGrid.getSelectedIndex()+1);
        sendSelEvt();
        }
}
    ...
    try{
        ...
        if(fgrd.isVisible()){
            filter = new UserActionLogFilter();
              ...
              filter.setId(!CheckNull.isEmpty(fid.getValue()) ? String.valueOf(fid.getValue()) : "0");
              filter.setBranch(fbranch.getValue());
              filter.setUser_id(!CheckNull.isEmpty(fuser_id.getValue()) ? String.valueOf(fuser_id.getValue()) : "0");
              ...
              filter.setAct_type(!CheckNull.isEmpty(fact_type.getValue()) ? String.valueOf(fact_type.getValue()) : "0");
              filter.setEntity_type(!CheckNull.isEmpty(fentity_type.getValue()) ? String.valueOf(fentity_type.getValue()) : "0");
