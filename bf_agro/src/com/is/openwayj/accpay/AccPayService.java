...
import com.is.openwayj.model.Address_old;
import com.is.openwayj.model.ApplicationContractAddress;
import com.is.openwayj.model.ApplicationReIssue;
import com.is.openwayj.model.ApplicationStatus;
import com.is.openwayj.model.ApplicationUpdContract;
import com.is.openwayj.model.DataAdd;
import com.is.openwayj.model.DataAddress;
import com.is.openwayj.model.DataCardReIssue;
import com.is.openwayj.model.DataCardStatus;
...
import com.is.openwayj.model.MsgDataContractAddress;
import com.is.openwayj.model.MsgDataInqContract;
import com.is.openwayj.model.MsgDataReIssue;
import com.is.openwayj.model.MsgDataUpdContract;
import com.is.openwayj.model.MsgDataUpdStatus;
...
public class AccPayService {
	...
	private static List<RefData> listSof; //source of founds
	...
	public static Res insert_sof(String p_branch, String p_client_id, int p_sof_id, String p_comment_txt, String un, String pwd, String alias)
    {
		Res res = new Res(0,"");
    	Connection c = null;
    	...
    	try {
            ...
            ps = c.prepareStatement("SELECT SQ_openway_confirm_sof.NEXTVAL id FROM DUAL");
            rs = ps.executeQuery();
            if (rs.next()) {
         	   v_id=rs.getInt("id");
            }
            ...
            ps = c.prepareStatement(
                	"begin "+
                    "  insert into bf_openway_confirm_sof(id, branch, user_id, client_id, "+
                    " sof_id, comment_txt, oper_date, sysd) "+
                    "  values (?,?,info.getempid,?, "+
                    "  ?,?,info.getday,sysdate); "+
                    "end; ");
            
			ps.setInt(1, v_id);
			ps.setString(2, p_branch);
			ps.setString(3, p_client_id);
			ps.setInt(4, p_sof_id);
			ps.setString(5, p_comment_txt);
	...
	public static String isMinibank_user(String branch, String userId) {
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			...
			com.is.ISLogger.getLogger().error("isMinibank_user "+branch+ "-"+ userId);
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("select count(*) cnt from ss_subsidiary_user us " +
					  "where us.branch = ? "+
					  "and us.id_user = ? ");
			ps.setString(1, branch);
	...
	public static String getShetKassa(String branch, String userId, String currency) {
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String acc="" ;
		try {
			com.is.ISLogger.getLogger().error("getShetKassa "+branch+ "-"+ userId+"-"+currency);
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("Select "+
                "ACC.ACC, "+
                "MB.NAME AS MBNAME, "+
                "BU.USER_NAME "+
               "From   SS_SUBSIDIARY_USER US, "+
               "       SS_SUBSIDIARY_ACC  ACC, "+
               "       SS_SUBSIDIARY      MB, "+
               "       USERS              BU "+
               "Where US.BRANCH = ? "+
                      "and US.ID_USER = ? "+
                      "and ACC.BRANCH = US.BRANCH "+
                      "and ACC.CODE = US.CODE "+
                      "and ACC.CUR = ? "+
                      "and MB.BRANCH = US.BRANCH "+
                      "and MB.CODE = US.CODE "+
                      "and MB.STATE = 'A' "+
                      "and BU.ID = US.ID_USER "+
                      "and BU.BRANCH = US.BRANCH");
			ps.setString(1, branch);
			ps.setString(2, userId);
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public static List<RefData> getSofList(final String alias) {
		if (listSof==null || listSof.size()==0)
		 listSof = (List<RefData>) Utils
				.getRefData(
						"select s.id data, s.id||' - '||s.name label from bf_openway_ss_sof s order by 1",
						alias);
		return listSof ;
	}
	...
	//bf_tr_payda bugungi kun b-n dokument bor yuqligini tekshiradigan metod
	public static boolean checkIfExistsCurrentDayPays(String alias)  {
        Connection c = null;
        PreparedStatement ps = null;
        CallableStatement cs = null;
        ResultSet rs = null;
        ...
        try {
             ...
             ps = c.prepareStatement("Select 1 "+
     				...
     				" and trunc(date_created) = trunc(info.getday) "+
     				" and rownum = ?");
	...
	//bf_sets ga 1(bir)ni qushub qaytarib beradigan metod
	public static Res getDocNumAndIncBf_sets(String alias) {
		String s = ConnectionPool.getValue("OPENWAY_INC_DOC_NUM");
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
        ...
        try {
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(
                "update bf_sets set value=? "+
                " where id=?");
            ps.setString(1, Integer.toString(Integer.parseInt(s)+1 ) );
            ps.setString(2, "OPENWAY_INC_DOC_NUM");
            ps.execute();
            c.commit();
            res = new Res(0, Integer.toString(Integer.parseInt(s)+1 ));            
        } 
        catch (Exception e) {
        	  try {
                  if (c != null)
	...
	//bf_sets ni inisializasiya qiladigan va 1 ni qaytarib beradigan metod
	public static Res getDocNumAndResetBf_sets(String alias) {
		//String s = ConnectionPool.getValue("OPENWAY_INC_DOC_NUM");
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
	...
	//umumiy metod shuerda yoki controller clasda buladi
	public static Res getNext_doc_num(String alias) {
		
		//1. bf_tr_payda bugungi operdata b-n dokument bor yuqligini tekshiramiz.
		//1.1. agar bor bulsa. bf_sets dan id= 'OPENWAY_INC_DOC_NUM' ning qiymatini olib unga 1(bir) ni qushib qaytaramiz. undan oldinroq bf_sets ni 1ga oshirib ham quyamiz. 
		//1.2. agar yuq bulsa. bf_sets da id='OPENWAY_INC_DOC_NUM' ni qiymatini 1 ga tenglashirib quyamiz va 1 ni qaytarib beramiz.
		
		if (checkIfExistsCurrentDayPays(alias)) {
			return getDocNumAndIncBf_sets(alias);
		} else {
			return getDocNumAndResetBf_sets(alias);
		}
    }
	...
	public static UFXMsgAddContractAddress makeAddSMS(AccInfo acc, String phoneMobile, String alias) {
		...
		Address_old addr = new Address_old();
