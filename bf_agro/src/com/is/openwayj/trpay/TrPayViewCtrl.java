...
import com.is.openwayj.model.UFXMsgPayAccResp;
...
public class TrPayViewCtrl extends GenericForwardComposer {
	private Div frm;
	private Listbox dataGrid, paytmpGrid, documentsGrid, logwnd$logGrid;
	...
	private Toolbarbutton btn_back, fbt, btn_ref$refwnd, btn_cancel$refwnd, btn_cancel$logwnd;
	private Toolbar tb;
	...
	private Window refwnd, printwnd, logwnd;
	...
	private static SimpleDateFormat mdf = new SimpleDateFormat("dd.MM.yyyy HH:mm:ss");
	private int uid = 0;
	...
	static String openwayEndpoint, url_way4_DebitCreditAcc;
	...
	public void doAfterCompose(Component comp) throws Exception {
		 ...
		 */
		//ISLogger.getLogger().error("doAfterCompose 001 " );
		...
		if (url_way4_DebitCreditAcc == null || url_way4_DebitCreditAcc == "" || url_way4_DebitCreditAcc.equals(""))
			url_way4_DebitCreditAcc = ConnectionPool.getValue("URL_WAY4_DEBITCREDITACC");
		...
		//ISLogger.getLogger().error("doAfterCompose 002 " );
		...
		//if (access > 0) {
			...
			filter.setParent_group_id(144);
		//}
			ISLogger.getLogger().error("doAfterCompose 004 " );
		dataGrid.setItemRenderer(new ListitemRenderer() {
			...
			public void render(Listitem row, Object data) throws Exception {
				...
				Listcell lc_parent_state = new Listcell();
				...
				Toolbarbutton bt_log_tieto = new Toolbarbutton();
				bt_log_tieto.setLabel("Детали...");
				bt_log_tieto.setImage("images/add-printer.png");
				bt_log_tieto.setAttribute("pTrPay", pTrPay);
				bt_log_tieto.addEventListener(Events.ON_CLICK,
						new EventListener() {
							@Override
							public void onEvent(Event event) throws Exception {
								// (TrPay)event.getTarget().getAttribute("pTrPay");
								// params.put("P_PAYID",
								// BigDecimal.valueOf(((TrPay) event
								// .getTarget().getAttribute("pTrPay")).getId()));

								logwnd$logGrid.setModel(new BindingListModelList(
										TrPayService
												.getRefillLogs(
														((TrPay) event
																.getTarget()
				...
				Toolbarbutton bt_act_tieto = new Toolbarbutton();
				bt_act_tieto.setLabel("Пополнить/списать в ТИЕТО");
				bt_act_tieto.setAttribute("pTrPay", pTrPay);
				
				bt_act_tieto.setAttribute("actid", 0);
				bt_act_tieto.setAttribute("act_name", "Пополнение/списание в ТИЕТО");
				bt_act_tieto.setAttribute("operation_desc", pTrPay.getOperation());
				bt_act_tieto.setAttribute("operation_id", pTrPay.getOperation_id());
				bt_act_tieto.setAttribute("action_desc", "Пополнение/списание в ТИЕТО");
				bt_act_tieto.setAttribute("deal_id", pTrPay.getDeal_id());
				
				bt_act_tieto.addEventListener(Events.ON_CLICK,
						new EventListener() {
							@Override
							public void onEvent(Event event) throws Exception {
								cur_cont = new Cur_cont(un, pwd, alias,
				...
				if (pTrPay.getParent_state() == null
						|| pTrPay.getParent_state().equals("")) {

				} else if (pTrPay.getParent_state().equals("0")) {
					Label st = new Label("Ошибка");
					lc_parent_state.appendChild(st);
					lc_parent_state.appendChild(bt_log_tieto);
					lc_parent_state.appendChild(bt_act_tieto);
				} else {
					Label st = new Label("Успешно");
					lc_parent_state.appendChild(st);
					lc_parent_state.appendChild(bt_log_tieto);
				}
				row.appendChild(lc_parent_state);
		...
		logwnd$logGrid.setItemRenderer(new ListitemRenderer() {
			@SuppressWarnings("unchecked")
			public void render(Listitem row, Object data) throws Exception {
				Refill_log paydoc = (Refill_log) data;
				row.setValue(paydoc);
				row.appendChild(new Listcell(paydoc.getUser_name()));
				row.appendChild(new Listcell(mdf.format(paydoc.getSysd())));
				row.appendChild(new Listcell(paydoc.getErr()));
			}
		});
	...
	private void refreshModel(int activePage) {
		...
		ISLogger.getLogger().error("refreshModel 002 " );
	}
	...
	public void authomat_step(Cur_cont cur_cont) {
		...
		try {
			...
			if (rs.getCode() < 0) {
				...
				if (TrPayService.isEndState(trpay1.getId(), alias, c)) {
					//if (send2way(trpay1)) {
					//	c.commit();
					//} else
					//	c.rollback();
					
					Res res=send2way(trpay1);
					Res res1 = com.is.openwayj.trpay.TrPayService
					.updateParentState(trpay1.getId(), (res.getCode() < 0) ? "0" : "1" );
					if (res1.getCode() < 0) {
						alert(res1.getName());
					}

					Refill_log r_log = new Refill_log();
					r_log.setPay_id(trpay1.getId());
					r_log.setUser_id("0");
					r_log.setUser_name(un);
					r_log.setUser_branch(branch1);
					if (res.getCode() < 0) {
						r_log.setErr(res.getName());
					} 
					Res res2 = com.is.openwayj.trpay.TrPayService
							.writeRefillLog(r_log);
					if (res2.getCode() < 0) {
						alert(res2.getName());
					}  
	...
	public void action_tieto(Cur_cont cur_cont) {
		...
		try {
			...
			Res res = /*com.is.openwayj.trpay.TrPayService.*/send2way(trpay1/*,
					alias*/);
	...
	public void onClick$btn_cancel$logwnd() {
	...
	private Res send2way(TrPay tp) {
		...
		DebitCreditAccRequest payAcc = TrPayService.makePayAcc(
				tp, alias, wayRrnConstant, wayPaySourceContractNum);

		// classni xml stringga aylantiramiz
		XmlUtils xmlUtils = new XmlUtils();
		try {
			v_json = xmlUtils.serializeJsonFromObject(payAcc);
			System.out.println("serializeJsonFromObject(DebitCreditAccRequest): " + v_json);
		} catch (Exception e) {
			System.out.println("error serialize json(DebitCreditAccRequest) "  + e.getMessage());
			//v_msg = v_msg + "\nError serialize xml(addContract) "
			//		+ e.getMessage();
			//alert ("error serialize json(DebitCreditAccRequest) "
			//		+ e.getMessage());
			//return false;
			res = new Res(-1, "Error serialize json " + e.getMessage());
			return res;
		}
		//ISLogger.getLogger().error("postUtils.sendData v_json : " + v_json);
		...
		Res v_res2 = null;
		try {
			v_res2 = postUtils.sendData(url_way4_DebitCreditAcc, v_json);
		} catch (Exception e) {
			...
			//return false;
			res = new Res(-1, "Error postUtils.sendData(DebitCreditAccRequest) " + e.getMessage());
			return res;
		}
		ISLogger.getLogger().error("postUtils.sendData way4_DebitCreditAcc v_res2: " + v_res2.getCode()+", "+v_res2.getName());
		
		if (v_res2.getCode()!=200) {
			ISLogger.getLogger().error( "Unsuccess response : "	+ v_res2.getCode()+", "+v_res2.getName());
			//alert("Unsuccess response : "	+ v_res2.getCode()+", "+v_res2.getName());
			//return false;
			res = new Res(-1, "Unsuccess response : "	+ v_res2.getCode()+", "+v_res2.getName());
			return res;
        }

		// kelgan javobdagi xml-stringdan java object (klass) yasaymiz
		DebitCreditAccResponse clResp = null;
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			clResp = objectMapper
					.readValue(v_res2.getName(), DebitCreditAccResponse.class);
		} catch (Exception e) {
			ISLogger.getLogger().error("objectMapper.readValue err (DebitCreditAccResponse): "
					+ e.getMessage());
			ISLogger.getLogger().error("objectMapper.readValue v_res2 (DebitCreditAccResponse): "
					+ v_res2);
			//alert("objectMapper.readValue err (DebitCreditAccResponse): "
			//		+ e.getMessage());
			//return false;
			res = new Res(-1, "Error DebitCreditAccRequest response mapper: " + e.getMessage());
			return res;
		}
		...
		if (clResp.isSuccess()) { /*--success*/
			...
			if (tp.getTieto_type()==110)
				...
				actType=2;
			UserService.WayQueryLog(new UserActionsLog(uid, uname, curip, actType, branch1, v_json,
					v_res2.getName()), alias);
			//return true;
			res = new Res(0, clResp.getDrn()); //getdrn nima qilaman uzim ham bilmayman hozircha
		} else {
			//v_msg = v_msg
			...
			//		+ clResp.getMessage());
			//return false;
			res = new Res(-1, "Ошибка пополнения/списания в Openway: " + clResp.getMessage());
			return res;
		}
		return res;
	}
