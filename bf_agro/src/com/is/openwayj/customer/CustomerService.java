...
import com.is.openwayj.model.ActivityPeriod;
import com.is.openwayj.model.AddContractInfo;
import com.is.openwayj.model.Application;
import com.is.openwayj.model.ApplicationContract;
import com.is.openwayj.model.ApplicationContractCard;
import com.is.openwayj.model.BaseAddress_old;
import com.is.openwayj.model.CardInfo;
import com.is.openwayj.model.Client_old;
import com.is.openwayj.model.ClientAdd;
import com.is.openwayj.model.ClientIDT;
import com.is.openwayj.model.ClientInfo;
import com.is.openwayj.model.ClientInfoIdt;
import com.is.openwayj.model.ContractAdd;
import com.is.openwayj.model.ContractIDT;

import com.is.openwayj.model.AccCardRequest;
import com.is.openwayj.model.AccCardResponse;
import com.is.openwayj.model.Account;
import com.is.openwayj.model.AccountAddress;
import com.is.openwayj.model.ApplicationUpdContract;
import com.is.openwayj.model.BalanceCardRequest;
import com.is.openwayj.model.BaseAddress;
import com.is.openwayj.model.Card;
import com.is.openwayj.model.CardInfoForBalance;
import com.is.openwayj.model.CardInfoRequest;
import com.is.openwayj.model.ClearPinRequest;
import com.is.openwayj.model.Client;
import com.is.openwayj.model.ClientAccCardRequest;
import com.is.openwayj.model.ClientContractsRequest;
import com.is.openwayj.model.ClientContractsResponse;
import com.is.openwayj.model.ClientInfoRequest;
import com.is.openwayj.model.ClientInfoResponse;
import com.is.openwayj.model.DataAccCard;
import com.is.openwayj.model.DataAdd;
import com.is.openwayj.model.DataContract;
import com.is.openwayj.model.DataRegCard;
import com.is.openwayj.model.DataUpdateSMS;
import com.is.openwayj.model.InformationContract;
import com.is.openwayj.model.LockCardRequest;
import com.is.openwayj.model.MsgDataAddClient;
import com.is.openwayj.model.MsgDataAddContract;
import com.is.openwayj.model.MsgDataAddContractCard;
import com.is.openwayj.model.MsgDataInqContract;
import com.is.openwayj.model.MsgDataUpdContract;
import com.is.openwayj.model.ObjectForAccCard;
import com.is.openwayj.model.ObjectForContract;
import com.is.openwayj.model.ObjectForRegCard;
import com.is.openwayj.model.ObjectForUpdateSMS;
import com.is.openwayj.model.Phone;
import com.is.openwayj.model.PhoneList;
import com.is.openwayj.model.PlasticInfo;
import com.is.openwayj.model.PlasticInfo_old;
import com.is.openwayj.model.ProductContract;
import com.is.openwayj.model.ProductionParms;
...
public class CustomerService {
	...
	private static List<AccInfo> listProductCode1Way4_2;
	private static HashMap<String, AccInfo> listMapProductCode;
	private static List<RefData> listSubProductCode1Way4;
	private static List<SubProduct> listSubProductCode1Way4_2;
	private static HashMap<String, SubProduct> listMapSubProductCode;
	...
	public static Res doAction(final String un, final String pw,
		...
		try {
			...
			cs.setString(2, (customer.getP_patronymic()!=null)? customer.getP_patronymic().toUpperCase() : "");
			cs.execute();
	...
	public static ClientInfoRequest makeClientInquiry(String pClientNumber, String pPinfl, String pUrl) {
		ClientInfoRequest cl = new ClientInfoRequest();
		cl.setSocialNumber(pPinfl);
		//cl.setRequestId("CLIENTINFO-065-"+mdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));	
		cl.setRequestId("CLIENTINFO-"+sdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));
		return cl;
	}
	
	public static ClientContractsRequest makeContractInquiry(String pClientNumber, String pPinfl, String pUrl) {
		ClientContractsRequest cl = new ClientContractsRequest();
		cl.setSocialNumber(pPinfl);
		cl.setRequestId("CLIENTCONTRACTS-"+sdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));
		return cl;
	}
	
	public static ClientAccCardRequest makeAddClient(Customer cust, AccInfo accInfo, CardInfo cardInfo, String alias, boolean is_test_mode) {
		ClientAccCardRequest addCl = new ClientAccCardRequest();
		//addCl.setRequestId("CL-ACC-CARD-065-"+mdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));
		addCl.setRequestId("CL-ACC-CARD-"+sdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));

        addCl.setOrderDprt( (is_test_mode) ? "01196" : cust.getOrder_dprt());		

        Client client = new Client();
		client.setClientType(cust.getO_client_type());
		client.setClientNumber(/*cust.getBranch()+*/cust.getId_client());
		client.setRegNumberType(getWayCertCode(cust.getP_type_document(), alias));
		client.setRegNumber(cust.getP_passport_serial()+cust.getP_passport_number());
		client.setRegNumberDetails(cust.getP_passport_place_registration()+", "+bdf.format(cust.getP_passport_date_registration())); 
		client.setSocialNumber(cust.getP_pinfl());
		client.setShortName((String)(cust.getP_family()+" "+cust.getP_first_name()+" "+cust.getP_patronymic()).trim());
		client.setTaxpayerIdentifier(cust.getP_number_tax_registration());
		client.setFirstName(cust.getP_first_name());
		client.setLastName(cust.getP_family());
		client.setMiddleName(cust.getP_patronymic());
		client.setSecurityName(cust.getO_security_name());
		client.setCountry(getWayCountryCode(cust.getCode_country(), alias));		
		client.setCitizenship(getWayCountryCode(cust.getP_code_citizenship(), alias));	
		...
		client.setBirthDate( df.format(cust.getP_birthday()));
		client.setBirthPlace(cust.getP_birth_place());
		//clInf.setBirthName(cust.get);
		client.setGender( (cust.getP_code_gender()=="1" || cust.getP_code_gender().equals("1")) ? "Male" : "Female" );
		...
		client.setPhoneList(phoneArrList);
		...
		baseAddr.setDateFrom(df.format(cust.getO_address_fact_date()));
		client.setBaseAddress(baseAddr);
		addCl.setClient(client);
		//Account
		Account account = new Account();
		account.setOrderDprt(/*cust.getBranch()*/addCl.getOrderDprt());
		//account.setRbsNumber("V_"+accInfo.getId());
		account.setRbsNumber(accInfo.getRbsNumberIbs());
		account.setContractName(accInfo.getContractName());
		account.setCommentText(accInfo.getCommentText());
		account.setProductCode(accInfo.getProductCode1());
		account.setDateOpen( df.format(Utils.getInfoDate(alias, new Res())).equals("2024-10-03") ?  "2025-01-10" : df.format(Calendar.getInstance().getTime())) ;
		account.setAddInfo1("CKC="+accInfo.getId()+";OVL=19997840600001196010");
		addCl.setAccount(account);
		AccountAddress accountAddr = new AccountAddress();
		accountAddr.setOrderDprt(/*cust.getBranch()*/addCl.getOrderDprt());
		accountAddr.setAddressType("SMS");
		accountAddr.setPostalCode(cust.getP_phone_mobile());
		addCl.setAccountAddress(accountAddr);
		//Card
		Card card = new Card();
		card.setOrderDprt(/*cust.getBranch()*/addCl.getOrderDprt());
		card.setRbsNumber(cardInfo.getRbsNumberIbs());
		card.setContractName(cardInfo.getContractName());
		card.setCommentText(cardInfo.getCommentText());
		card.setProductCode(cardInfo.getProductCode1());
		card.setFirstName(cardInfo.getFirstName());
		card.setLastName(cardInfo.getLastName());
		//card.setDateOpen(df.format(Calendar.getInstance().getTime()));
		card.setDateOpen( df.format(Utils.getInfoDate(alias, new Res())).equals("2024-10-03") ?  "2025-01-10" : df.format(Calendar.getInstance().getTime())) ;
		addCl.setCard(card);
		AccountAddress cardAddr = new AccountAddress();
		cardAddr.setOrderDprt(/*cust.getBranch()*/addCl.getOrderDprt());
		cardAddr.setAddressType("SMS");
		cardAddr.setPostalCode(cust.getP_phone_mobile());
		addCl.setCardAddress(cardAddr);
		
		return addCl;

	}
	
	public static UFXMsgUpdClient makeUpdClient_old_xml_version(Customer cust, String alias) {
		...
		PlasticInfo_old plInfo = new PlasticInfo_old();
		plInfo.setFirstName(cust.getP_first_name());
		plInfo.setLastName(cust.getP_family());
		client.setPlasticInfo(plInfo);
		PhoneList phoneList = new PhoneList();
		...
		//client.setDateOpen(df.format(Calendar.getInstance().getTime()) );
		client.setDateOpen(df.format(Utils.getInfoDate(alias, new Res())));
		BaseAddress_old baseAddr = new BaseAddress_old();
		baseAddr.setEMail(cust.getP_email_address());
		baseAddr.setCity(cust.getO_city());
		baseAddr.setPostalCode(cust.getP_zip_code());
		baseAddr.setAddressLine1(cust.getP_post_address());
	...
	public static UFXMsgAddContractAcc makeAddContract_old(AccInfo acc, String alias) {
	...
	public static AccCardRequest makeAddContract(AccInfo acc, CardInfo card, String alias) {
		AccCardRequest addCl = new AccCardRequest();
		addCl.setRequestId("ACC-CARD-"+sdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));
		addCl.setOrderDprt(acc.getOrder_dprt());
		
		ObjectForAccCard obj = new ObjectForAccCard();
		obj.setClientNumber(acc.getClient());
		obj.setSocialNumber(acc.getSocialNumber());
		DataAccCard data = new DataAccCard();
		data.setRbsNumber(acc.getRbsNumberIbs());
		data.setContractName(acc.getContractName());
		data.setCommentText(acc.getCommentText());
		data.setProductCode(acc.getProductCode1());
		data.setDateOpen(df.format(Calendar.getInstance().getTime()));
		data.setAddInfo1("CKC="+acc.getId()+";OVL=19997840600001196010");
		obj.setData(data);
		addCl.setObjectFor(obj);
		AccountAddress addr = new AccountAddress();
		addr.setOrderDprt(addCl.getOrderDprt());
		addr.setAddressType("SMS");
		addr.setPostalCode(acc.getPhone());
		addCl.setAccountAddress(addr);
		Card card1= new Card();
		card1.setOrderDprt(addCl.getOrderDprt());
		card1.setRbsNumber(card.getRbsNumberIbs());
		card1.setContractName(card.getContractName());
		card1.setCommentText(card.getCommentText());
		card1.setProductCode(card.getProductCode1());
		card1.setFirstName(card.getFirstName());
		card1.setLastName(card.getLastName());
		card1.setDateOpen( df.format(Utils.getInfoDate(alias, new Res())).equals("2024-10-03") ?  "2025-01-10" : df.format(Calendar.getInstance().getTime())) ;
		addCl.setCard(card1);
		
		AccountAddress cardAddr = new AccountAddress();
		cardAddr.setOrderDprt(/*cust.getBranch()*/addCl.getOrderDprt());
		cardAddr.setAddressType("SMS");
		cardAddr.setPostalCode(card.getPhone());
		addCl.setCardAddress(cardAddr);
	...
	public static UFXMsgAddContractCard makeAddContractCard_old(CardInfo card, String alias) {
	...
	public static RegCardRequest makeAddContractCard(CardInfo card, /*AccInfo acc*/ String accRbsNumber, String alias) {
		RegCardRequest addCl = new RegCardRequest();
		addCl.setRequestId("CARD-"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		addCl.setOrderDprt( card.getOrder_dprt());
		ObjectForRegCard objectFor = new ObjectForRegCard();
		objectFor.setRbsNumber(/*acc.getCbsNumber()*/accRbsNumber);
		objectFor.setClientNumber(card.getClient_id());
		objectFor.setSocialNumber(card.getSocialNumber());
		DataRegCard data = new DataRegCard();
		data.setRbsNumber(card.getRbsNumberIbs());
		data.setContractName(card.getContractName());
		data.setCommentText(card.getCommentText());
		data.setProductCode(card.getProductCode1());
		data.setDateOpen(df.format(Calendar.getInstance().getTime()));
		data.setFirstName(card.getFirstName());
		data.setLastName(card.getLastName());
		objectFor.setData(data);
		addCl.setObjectFor(objectFor);
		AccountAddress cardAddress = new AccountAddress();
		cardAddress.setOrderDprt(card.getOrder_dprt());
		cardAddress.setAddressType("SMS");
		cardAddress.setPostalCode(card.getPhone());
		addCl.setCardAddress(cardAddress);
		return addCl;
	}
	
	public static LockCardRequest makeLockCard(CardInfo card, String alias) {
		LockCardRequest addCl = new LockCardRequest();
		addCl.setRequestId("LOCKCARD-"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		addCl.setOrderDprt(card.getOrder_dprt());
		addCl.setRbsNumber(card.getRbsNumberIbs());
		addCl.setSocialNumber(card.getSocialNumber());
		addCl.setStatus(card.getSTOP_CAUSE());
		addCl.setStatusComment(card.getCommentText());
		return addCl;
	}
	
	public static ClearPinRequest makeClearPinRequest(CardInfo card, String alias) {
		ClearPinRequest addCl = new ClearPinRequest();
		addCl.setRequestId("CLEARPIN-"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		addCl.setRbsNumber(card.getRbsNumberIbs());
		addCl.setSocialNumber(card.getSocialNumber());
	...
	public static CardInfoRequest makeCardInfo(CardInfo card, String alias) {
		CardInfoRequest addCl = new CardInfoRequest();
		addCl.setRequestId("CARDINFO-"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		addCl.setRbsNumber(card.getRbsNumberIbs());
		return addCl;
	...
	public static UpdateSMSRequest makeAddSMS_card_acc (CardInfo card, String accRbsNumber, String accPhone, String curr_office_number, String alias) {
		UpdateSMSRequest addSms = new UpdateSMSRequest();
		addSms.setRequestId("SMSSET-"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		ObjectForUpdateSMS obj = new ObjectForUpdateSMS();
		
		ArrayList < DataUpdateSMS > datas = new ArrayList<DataUpdateSMS>();
		DataUpdateSMS data1 = new DataUpdateSMS();//account sms
		data1.setClientNumber(card.getClient_id());
		data1.setSocialNumber(card.getSocialNumber());
		data1.setRbsNumber(accRbsNumber);
		String accOrderDprt = getAccOrderDprt(card.getBranch(), card.getClient_id(), accRbsNumber, alias);
		if (accOrderDprt==null || accOrderDprt.equals(""))
			accOrderDprt=curr_office_number;
		data1.setOrderDprt(accOrderDprt);
		data1.setPostalCode(accPhone);
		datas.add(data1);
		
		data1 = new DataUpdateSMS();//card sms
		data1.setClientNumber(card.getClient_id());
		data1.setSocialNumber(card.getSocialNumber());
		data1.setRbsNumber(card.getRbsNumberIbs());
		data1.setOrderDprt(card.getOrder_dprt()!=null ? card.getOrder_dprt() : curr_office_number);
		data1.setPostalCode(accPhone);
		datas.add(data1);
		
		obj.setDatas(datas);
		addSms.setObjectFor(obj);
		return addSms;
	}
	
	public static UpdateSMSRequest makeAddSMS_acc (AccInfo card, String accPhone, String curr_office_number, String alias) {
		UpdateSMSRequest addSms = new UpdateSMSRequest();
		addSms.setRequestId("SMSSET -"+sdf.format(Calendar.getInstance().getTime())+"-"+
	             String.format("%04d", getSeqApplRegNumber()) 
		);
		...
		ArrayList < DataUpdateSMS > datas = new ArrayList<DataUpdateSMS>();
		DataUpdateSMS data1 = new DataUpdateSMS();//account sms
		data1.setClientNumber(card.getClient());
		data1.setSocialNumber(card.getSocialNumber());
		data1.setRbsNumber(card.getCbsNumber());
		String accOrderDprt = getAccOrderDprt(card.getBranch(), card.getClient(), card.getCbsNumber(), alias);
		if (accOrderDprt==null || accOrderDprt.equals(""))
			accOrderDprt=curr_office_number;
		data1.setOrderDprt(accOrderDprt);
		data1.setPostalCode(accPhone);
    ...
    public static UFXMsgInqContract makeClassForBalance_old (String cardRbsNumber) {
	...
	public static BalanceCardRequest makeClassForBalance (String cardRbsNumber) {
		
		BalanceCardRequest bal = new BalanceCardRequest();
		bal.setRequestId("BALANCE-"+sdf.format(Calendar.getInstance().getTime())+"-"+ String.format("%04d", getSeqApplRegNumber()));
 
		List < CardInfoForBalance > card_info = new ArrayList<CardInfoForBalance>();
		CardInfoForBalance c1= new CardInfoForBalance();
		c1.setRbsNumber(cardRbsNumber);
		card_info.add(c1);
		bal.setCard_info(card_info);
	...
	public static ClientInfoResponse getCustomers_openway(String pBranch, String pClientId, String pPinfl, String pUrl, final String alias) {
		...
		if (pPinfl==null || pPinfl=="" || pPinfl.equals("")){
			clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("Параметр ПИНФЛ пустой.(IBS)");
			return clResp;
		}
		
		// request uchun xml string tayyorlaymiz
		// CustomerFilter ni bersak ClientInquiry klass yasaydigan funksiya
		ClientInfoRequest cl = makeClientInquiry(pBranch + pClientId, pPinfl, pUrl);
		// ba'zi polyalarni urnatamiz agar kerak bulsa

		// class dan xml(json) string qaytaradigan funksiya
		XmlUtils xmlUtils = new XmlUtils();
		String v_json = "_";
		try {
			v_json = xmlUtils.serializeJsonFromObject(cl);
		} catch (Exception e) {
			
			System.out.println("error serialize json " + e.getMessage());
			ISLogger.getLogger().error("error serialize json " + e.getMessage());
			
			if (clResp==null)
				clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("error serialize xml(IBS) " + e.getMessage());
			return clResp;
		}
		...
		Res v_res2 = null;
		try {
			v_res2 = postUtils.sendData(pUrl, v_json);
			ISLogger.getLogger().error("getCustomers_openway v_res2: " + v_res2.getCode()+", "+v_res2.getName());
		} catch (Exception e) {
			ISLogger.getLogger().error("postUtils.sendData err " + e.getMessage());
			if (clResp==null)
				clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("postUtils.sendData err(IBS) " + e.getMessage());
			return clResp;
		}
		...
		if (v_res2.getCode()!=200){
			if (clResp==null)
				clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("Response (WAY) " + v_res2.getCode()+", "+v_res2.getName());
			ISLogger.getLogger().error("Response (WAY) " + v_res2.getCode()+", "+v_res2.getName());
			return clResp;
		}

		
		//v_res2 shunaqa qiymat yoki uxshashi kelsa xatoki bulganini bildiradi
		//"null, code: 409"
		//Bad Gateway, code: 502
		if (v_res2.getName().contains("null, code: ") || v_res2.getName().contains("Bad Gateway, code: ")){
			if (clResp==null)
				clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("Response (WAY) " +v_res2.getCode()+", "+v_res2.getName());
		...
		try {
			clResp = objectMapper.readValue(v_res2.getName(), ClientInfoResponse.class);
		} catch (Exception e) {
			ISLogger.getLogger().error("objectMapper.readValue err " + e.getMessage());
			if (clResp==null)
				clResp= new ClientInfoResponse();
			clResp.setSuccess(false);
			clResp.setMessage("objectMapper.readValue err(IBS) " + e.getMessage());
			return clResp;
		}
	...
	public static ClientContractsResponse getCustomersContract_openway(String pBranch, String pClientId, String pPinfl, String pUrl, final String alias) {
		ClientContractsResponse clResp = null;
		
		// maqsad list of customers yoki bitta listni qaytarib berishimiz kerak
		// request uchun xml string tayyorlaymiz
		// CustomerFilter ni bersak ClientInquiry klass yasaydigan funksiya
		ClientContractsRequest cl = makeContractInquiry( pBranch + pClientId, pPinfl, pUrl);
		// ba'zi polyalarni urnatamiz agar kerak bulsa
		...
		try {
			...
			if (clResp==null)
				clResp= new ClientContractsResponse();
			clResp.setSuccess(false);
			clResp.setMessage("error serialize xml(IBS) " + e.getMessage());
			return clResp;
		}
		...
		try {
			v_res2 = postUtils.sendData(pUrl, v_json);
			ISLogger.getLogger().error("getCustomersContract_openway v_res2: " + v_res2.getCode()+", "+v_res2.getName());
		} catch (Exception e) {
			if (clResp==null)
				clResp= new ClientContractsResponse();
			clResp.setSuccess(false);
        ...
        ObjectMapper objectMapper = new ObjectMapper();
		try {
			clResp = objectMapper.readValue(v_res2.getName(), ClientContractsResponse.class);
		} catch (Exception e) {
			ISLogger.getLogger().error("objectMapper.readValue err " + e.getMessage());
			if (clResp==null)
				clResp= new ClientContractsResponse();
	...
	public static List<Customer> getCustomersFl(final int pageIndex, final int pageSize, final CustomerFilter filter, final String alias) {
		...
		try {
			...
			while (rs.next()) {
                ...
                customer.setOrder_dprt(rs.getString("office_number"));
	...
	public static Res insertClient_oldVersion(Customer client) {
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
        //boolean bool = false;
        try {
            ISLogger.getLogger().error("insert/update bf_openway_clients");
            ...
            ps = c.prepareStatement(
            		...
            		"office_number=nvl(office_number,?) "+
            		"where branch=? and id_client=?; if sql%notfound then "+
            		"insert into bf_openway_clients(o_client_type,o_security_name,"
            		                    + "o_city,o_post_address_fact,o_address_fact_date,office_number,date_open,branch,id_client) values"
            		                    + " (?,?,"
            ...
            ps.setString(6, client.getOrder_dprt());
            ps.setString(7, client.getBranch());
            ps.setString(8, client.getId_client());
            
            ps.setString(9, client.getO_client_type());
            ps.setString(10, client.getO_security_name());
            ps.setString(11, client.getO_city());
            ps.setString(12, client.getO_post_address_fact());
            ps.setDate(13,  new java.sql.Date(client.getO_address_fact_date().getTime()));
            ps.setString(14, client.getOrder_dprt());
            ps.setString(15, client.getBranch());
            ps.setString(16, client.getId_client());
	...
	public static Res insertClient(Customer client) {
        ...
        try {
            ...
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(
            		"insert into bf_openway_clients(branch, id_client, office_number, o_client_type, "
       		                 + "o_security_name, o_city, o_post_address_fact, o_address_fact_date, "
       		                 + "address_line, code_citizenship, code_gender, type_document, "
       		                 + "passport_serial, passport_number, passport_place_registration, passport_date_registration, "
       		                 + "passport_date_expiration, postal_code, taxpayer_identifier, first_name, "
       		                 + "last_name, middle_name, work_phone_number, mobile_phone_number, "
       		              	 + "email, date_open"
       		                 +") values"
       		                 + " (?,?,?,?,"
       		                 + "?,?,?,?,"
            ...
            ps.setString(3, client.getOrder_dprt());
            ps.setString(4, client.getO_client_type());
            ps.setString(5, client.getO_security_name());
            ps.setString(6, client.getO_city());
            ps.setString(7, client.getO_post_address_fact());
            ps.setDate(8, new java.sql.Date(client.getO_address_fact_date().getTime()));
            ps.setString(9, client.getP_post_address());
            ps.setString(10, client.getP_code_citizenship());
            ps.setString(11, client.getP_code_gender());
            ps.setString(12, client.getP_type_document());
            ps.setString(13, client.getP_passport_serial());
            ps.setString(14, client.getP_passport_number());
            ps.setString(15, client.getP_passport_place_registration());
            ps.setDate(16, new java.sql.Date(client.getP_passport_date_registration().getTime()));
            ps.setDate(17, new java.sql.Date(client.getP_passport_date_expiration().getTime()));
            ps.setString(18, client.getP_zip_code());
            ps.setString(19, client.getP_number_tax_registration());
            ps.setString(20, client.getP_first_name());
            ps.setString(21, client.getP_family());
            ps.setString(22, client.getP_patronymic());
            ps.setString(23, client.getP_phone_home());
            ps.setString(24, client.getP_phone_mobile());
            ps.setString(25, client.getP_email_address());
	...
	public static Res updateClient(Customer client) {
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
        //boolean bool = false;
        try {
            ISLogger.getLogger().error("update bf_openway_clients");
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(

            		"update bf_openway_clients set "+
            		"o_client_type=?, o_security_name=?, o_city=?, o_post_address_fact=?, "+
            		"o_address_fact_date=?, office_number=nvl(office_number,?), code_citizenship=?, code_gender=?, "+
            		"type_document=?, passport_serial=?, passport_number=?, passport_place_registration=?,"+
            		"passport_date_registration=?, passport_date_expiration=?, address_line=?, taxpayer_identifier=?,"+
            		"first_name=?, last_name=?, middle_name=?, work_phone_number=?,"+
            		"mobile_phone_number=?, email=?, postal_code=? "+
            ...
            ps.setDate(5,  new java.sql.Date(client.getO_address_fact_date().getTime()));
            ps.setString(6, client.getOrder_dprt());
            ps.setString(7, client.getP_code_citizenship());
            ps.setString(8, client.getP_code_gender());
            
            ps.setString(9, client.getP_type_document());
            ps.setString(10, client.getP_passport_serial());
            ps.setString(11, client.getP_passport_number());
            ps.setString(12, client.getP_passport_place_registration());

            ps.setDate(13, new java.sql.Date(client.getP_passport_date_registration().getTime()));
            ps.setDate(14, new java.sql.Date(client.getP_passport_date_expiration().getTime()));
            ps.setString(15, client.getP_post_address());
            ps.setString(16, client.getP_number_tax_registration());

            ps.setString(17, client.getP_first_name());
            ps.setString(18, client.getP_family());
            ps.setString(19, client.getP_patronymic());
            ps.setString(20, client.getP_phone_home());

            ps.setString(21, client.getP_phone_mobile());
            ps.setString(22, client.getP_email_address());
            ps.setString(23, client.getP_zip_code());
            
            ps.setString(24, client.getBranch());
            ps.setString(25, client.getId_client());
	...
	public static Res insertAccount(AccInfo acc) {
        ...
        try {
            ...
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(
            		"begin update bf_openway_contract_acc set contract_name=?, comment_text=?, product_code=?, currency=nvl(currency,?), date_open=nvl(info.getday,sysdate), "+
            		"office_number=nvl(office_number,?), phone=? "+
            		"where branch=? and id_client=? and rbs_number=?; if sql%notfound then "+
            		"insert into bf_openway_contract_acc(contract_name,comment_text,product_code,currency,office_number,phone,date_open,branch,id_client,rbs_number) values("
            		                    + "?, ?, ?, ?, ?, ?, nvl(info.getday,sysdate), ?, ?, ?); "+
            		"end if; "+
            		"end;"
            );
            ...
            ps.setString(5, acc.getOrder_dprt());
            ps.setString(6, acc.getPhone());
            ps.setString(7, acc.getBranch());
            ps.setString(8, acc.getClient());
            ps.setString(9, acc.getRbsNumberIbs());
            
            ps.setString(10, acc.getContractName());
            ps.setString(11, acc.getCommentText());
            ps.setString(12, acc.getProductCode1());
            ps.setString(13, acc.getCurrency());
            ps.setString(14, acc.getOrder_dprt());
            ps.setString(15, acc.getPhone());
            ps.setString(16, acc.getBranch());
            ps.setString(17, acc.getClient());
            ps.setString(18, acc.getRbsNumberIbs());
	...
	public static Res updateAccount(AccInfo acc) {
        ...
        //boolean bool = false;
        try {
            ISLogger.getLogger().error("update bf_openway_contract_acc");
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(
  ...
  public static List<CardInfo> getContractCardList_ABS(final String pBranch, final String pId_client, final String alias) {
		...
		try {
			...
			while (rs.next()) {
                ...
                card.setRbsNumberIbs(rs.getString("rbs_number"));
                card.setCbsNumber(rs.getString("cbs_number"));
                card.setContractName(rs.getString("contract_name"));
	...
	public static Res insertCard(CardInfo card) {
        ...
        try {
            ...
            c = ConnectionPool.getConnection();
            ps = c.prepareStatement(
            		"begin update bf_openway_contract_card set contract_name=?, comment_text=?, "+
            		" currency=nvl(currency,?), product_name=nvl(product_name,?), product_code_ibs=nvl(product_code_ibs,?),"+
            		//" date_open=nvl(info.getday,sysdate), office_number=nvl(office_number,?) "+
            		" date_open=nvl(info.getday,sysdate), office_number=decode(nvl(cbs_number,'_'), '_',?, office_number) "+  //cbs_number null bulsagina office_numberni uzgartiradi
            		"where branch=? and id_client=? and product_code=? and sub_product_code=? and rbs_number=?; if sql%notfound then "+
            		"insert into bf_openway_contract_card(contract_name,comment_text,currency,product_name,product_code_ibs,office_number,"+
            		 "date_open,branch,id_client,product_code,sub_product_code,rbs_number) values("
            		                    + "?, ?, ?, ?, ?, ?, nvl(info.getday,sysdate), ?, ?, ?, ?, ?); "+
            		"end if; "+
            		"end;"
            );
            ...
            ps.setString(3, card.getCurrency());
            ps.setString(4, card.getProductWay());
            ps.setString(5, card.getProductIbs());
            ps.setString(6, card.getOrder_dprt());
            ps.setString(7, card.getBranch());
            ps.setString(8, card.getClient_id());
            ps.setString(9, card.getMasterProductCode1());
            ps.setString(10, card.getProductCode1());
            ps.setString(11, card.getRbsNumberIbs());
            
            ps.setString(12, card.getContractName());
            ps.setString(13, card.getCommentText());
            ps.setString(14, card.getCurrency());
            ps.setString(15, card.getProductWay());
            ps.setString(16, card.getProductIbs());
            ps.setString(17, card.getOrder_dprt());
            ps.setString(18, card.getBranch());
            ps.setString(19, card.getClient_id());
            ps.setString(20, card.getMasterProductCode1());
            ps.setString(21, card.getProductCode1());
            ps.setString(22, card.getRbsNumberIbs());
	...
	public static Res activateSmsService_card_acc(CardInfo card, String phoneNumber, String accRbsNumber, String accPhoneNumber) {
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            ps = c.prepareStatement(
                ...
                " values (?,?,?,?,'A');  "+
            	"end; ");
            ps.setString(1, accPhoneNumber);
            ps.setString(2, card.getBranch());
            ps.setString(3, card.getClient_id());
            ps.setString(4, accRbsNumber);
            ps.setString(5, accPhoneNumber);
            ps.setString(6, card.getBranch());
            ps.setString(7, card.getClient_id());
            ps.setString(8, accRbsNumber);
            ps.setString(9, accPhoneNumber);
            ps.setString(10, card.getBranch());
            ps.setString(11, card.getClient_id());
            ps.setString(12, accRbsNumber);

            ps.setString(13, phoneNumber);
            ps.setString(14, card.getBranch());
            ps.setString(15, card.getClient_id());
            ps.setString(16, card.getRbsNumberIbs());
            ps.setString(17, phoneNumber);
            ps.setString(18, card.getBranch());
            ps.setString(19, card.getClient_id());
            ps.setString(20, card.getRbsNumberIbs());
            ps.setString(21, phoneNumber);
            ps.setString(22, card.getBranch());
            ps.setString(23, card.getClient_id());
            ps.setString(24, card.getRbsNumberIbs());
	...
	public static Res activateSmsService_acc(AccInfo card, String phoneNumber) {
		Res res = null;
        Connection c = null;
        PreparedStatement ps = null;
        ...
        try {
            ...
            ps = c.prepareStatement(
                ...
                " insert into bf_openway_sms_hist(phone_number, branch, id_client, rbs_number, act ) "+ 
                " values (?,?,?,?,'A');  "+
                " update bf_openway_contract_acc set phone=? "+
                "  where branch=? and id_client=? and rbs_number=?; "+
            	"end; ");
            ps.setString(1, phoneNumber);
            ps.setString(2, card.getBranch());
            ps.setString(3, card.getClient());
            ps.setString(4, card.getCbsNumber());
            ps.setString(5, phoneNumber);
            ps.setString(6, card.getBranch());
            ps.setString(7, card.getClient());
            ps.setString(8, card.getCbsNumber());
            ps.setString(9, phoneNumber);
            ps.setString(10, card.getBranch());
            ps.setString(11, card.getClient());
            ps.setString(12, card.getCbsNumber());
            ps.setString(13, phoneNumber);
            ps.setString(14, card.getBranch());
            ps.setString(15, card.getClient());
            ps.setString(16, card.getCbsNumber());
	...
	public static String getCardSmsPhoneNumber(final String branch, String client, String rbs_number) {
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("select phone_number from bf_openway_sms where branch = ? and id_client = ? and rbs_number=?");
			ps.setString(1, branch);
			ps.setString(2, client);
			ps.setString(3, rbs_number);
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public static String getCardSmsPhoneNumber2(final String branch, String client, String rbs_number) {

		//agar bf_openway_sms da usha rbs buyicha bor bulsa ushani kursatamiz (phone polya bush bulsa ham kursatamiz)
		//yuq esa bf_openway_contract_acc dan kursatamiz
		...
		// bf_openway_contract_acc

		if (rbs_number==null || rbs_number.length()<=20)
			return phoneNumber;
		String acc_rbs_number=rbs_number.substring(rbs_number.length()-20,rbs_number.length());
		acc_rbs_number="V_"+acc_rbs_number;
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("select phone from BF_OPENWAY_CONTRACT_ACC where branch = ? and id_client = ? and rbs_number=?");
			ps.setString(1, branch);
			ps.setString(2, client);
			ps.setString(3, acc_rbs_number);
			rs = ps.executeQuery();
			if (rs.next()) {
				phoneNumber = rs.getString(1);
			}
	...
	public static String getAccOrderDprt(final String branch, final String customerId, final String accRbsNumber, final String alias) {
		String v_res = null;
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("SELECT OFFICE_NUMBER FROM BF_OPENWAY_CONTRACT_ACC WHERE branch=? and id_client=? and RBS_NUMBER=?");
			ps.setString(1, branch);
			ps.setString(2, customerId);
			ps.setString(3, accRbsNumber);
			
			rs = ps.executeQuery();
			if (rs.next()) {
				v_res = rs.getString("OFFICE_NUMBER");
			}
		} catch (Exception e) {
			ISLogger.getLogger().error((Object) CheckNull.getPstr(e));
			e.printStackTrace();
	...
	public static List<RefData> getListLockStopCauses(String alias) {
		if (listLockStopCauses == null || listLockStopCauses.size() == 0)
			listLockStopCauses = (List<RefData>) Utils
					.getRefData(
							"select code data, code||'-'||name label from bf_openway_stop_cause where act='L' order by 1",
							alias);
		return listLockStopCauses;
	}
	
	public static int getCountDepartmentByUserID(String branch, String user_id, final String alias) {
		int res=-1;
		...
		try {
			c = ConnectionPool.getConnection(alias);
			ps = c.prepareStatement("select count(*) from ss_subsidiary_user where branch=? and id_user=?");
			ps.setString(1, branch);
			...
			ISLogger.getLogger().error((Object) CheckNull.getPstr((Exception) e));
			e.printStackTrace();
			ISLogger.getLogger().error("ss_subsidiary_user Пользовател("+user_id+") count SQLException =>"
					+ e.getMessage());
			return res;
		} finally {
			try {
	...
	public static String getDepartmentByUserID(String branch, String user_id, final String alias) {
		String res=null;
		...
		try {
			...
			ISLogger.getLogger().error((Object) CheckNull.getPstr((Exception) e));
			e.printStackTrace();
			ISLogger.getLogger().error("ss_subsidiary_user Пользовател("+user_id+") select SQLException =>"
					+ e.getMessage());
			return "Error.";
		} finally {
			try {
		...
		listClientTypeWay4 = (List<RefData>) Utils
				.getRefData(
						"select 'PR' data,	'Физическое лицо - резидент(PR)' label, 0 ord from dual union all "
								...
								+ "select 'RPR' data, 'Физическое лицо, пенсионер(RPR)' label, 2 ord from dual union all "
								+ "select 'CR' data, 'Юридическое лицо - резидент(CR)' label, 3 ord from dual order by ord",								
						alias);
	...
	public static List<RefData> getProduct_code1_way4(final String alias) {
		if (listProductCode1Way4==null || listProductCode1Way4.size()==0){
			...
			//			alias);
			if (listProductCode1Way4_2==null || listProductCode1Way4_2.size()==0){
				listProductCode1Way4_2=getListProductCode_way4(alias);	
			}
			listProductCode1Way4 = new ArrayList<RefData>();
			for (int i = 0; i < listProductCode1Way4_2.size(); i++) {
			    listProductCode1Way4.add(new RefData(listProductCode1Way4_2.get(i).getProductCode1(), listProductCode1Way4_2.get(i).getProduct_name()));
			}
		}
		return listProductCode1Way4;
	}
	
    public static List<AccInfo> getListProductCode_way4(final String alias) {
		...
		try {
			c = ConnectionPool.getConnection(alias);
			ps = c.prepareStatement("select CODE_PROD_WAY, NAME_PROD_IBS, type_prod from bf_openway_product where state='A' order by 1");
			rs = ps.executeQuery();

			if (listMapProductCode == null ) 
				listMapProductCode = new HashMap<String, AccInfo>();
			
			while (rs.next()) {
                final AccInfo acc = new AccInfo();
                acc.setProductCode1(rs.getString("CODE_PROD_WAY")); //name
                acc.setCurrency(rs.getString("type_prod"));  //currency 
                acc.setProduct_name(rs.getString("NAME_PROD_IBS")); //label
				list.add(acc);
				listMapProductCode.put(rs.getString("CODE_PROD_WAY"), acc);
			}
		} catch (SQLException e) {
			ISLogger.getLogger().error((Object) CheckNull.getPstr((Exception) e));
			e.printStackTrace();
			ISLogger.getLogger().error("getListProductCode_way4 SQLException =>"
					+ e.getMessage());
			return list;
		} finally {
			try {
	...
	public static AccInfo getMapProduct_code1_way4(final String product_code, final String alias) {
		return listMapProductCode.get(product_code);
	}
	...
	public static List<RefData> getSubProduct_code1_way4(final String alias) {
		if (listSubProductCode1Way4==null || listSubProductCode1Way4.size()==0){
			...
			//			"select CODE_SUBPROD_WAY data, NAME_SUBPROD_IBS||' - '||CODE_SUBPROD_WAY label, 0 ord from bf_openway_subproduct where state='A' order by CODE_SUBPROD_WAY",
			//			alias);
			if (listSubProductCode1Way4_2==null || listSubProductCode1Way4_2.size()==0){
				listSubProductCode1Way4_2=getListSubProductCode_way4(alias);	
			}
			listSubProductCode1Way4 = new ArrayList<RefData>();
			for (int i = 0; i < listSubProductCode1Way4_2.size(); i++) {
			    listSubProductCode1Way4.add(new RefData(listSubProductCode1Way4_2.get(i).getCode_subprod_way(), listSubProductCode1Way4_2.get(i).getName_subprod_ibs()/*+" - "+listSubProductCode1Way4_2.get(i).getCode_subprod_way()*/));
			}
		}
		return listSubProductCode1Way4;
	}
	
	public static List<SubProduct> getListSubProductCode_way4(final String alias) {
		
		final List<SubProduct> list = new ArrayList<SubProduct>();
		Connection c = null;
		...
		try {
			c = ConnectionPool.getConnection(alias);
			ps = c.prepareStatement("select CODE_SUBPROD_WAY, NAME_SUBPROD_IBS, name_subprod_way from bf_openway_subproduct where state='A' order by 1");
			rs = ps.executeQuery();

			if (listMapSubProductCode == null ) 
				listMapSubProductCode = new HashMap<String, SubProduct>();
			
			while (rs.next()) {
                final SubProduct acc = new SubProduct();
                acc.setCode_subprod_way(rs.getString("CODE_SUBPROD_WAY"));
                acc.setName_subprod_ibs(rs.getString("NAME_SUBPROD_IBS"));
                acc.setName_subprod_way(rs.getString("name_subprod_way"));
				list.add(acc);
				listMapSubProductCode.put(rs.getString("CODE_SUBPROD_WAY"), acc);
			}
		} catch (SQLException e) {
			ISLogger.getLogger().error((Object) CheckNull.getPstr((Exception) e));
			e.printStackTrace();
			ISLogger.getLogger().error("getListSubProductCode_way4 SQLException =>"
					+ e.getMessage());
			return list;
		} finally {
			try {
	...
	public static SubProduct getMapSubProduct_code1_way4(final String subProduct_code, final String alias) {
		return listMapSubProductCode.get(subProduct_code);
	}
	...
	public static List<RefData> getSubProductByProduct(String product_code, final String alias) {
		...
		if ( !listSubProductByProduct.containsKey(product_code) ) {
			List<RefData> lst = (List<RefData>) Utils
			.getRefData(
					"select CODE_SUBPROD_WAY data, NAME_SUBPROD_IBS||' - '||CODE_SUBPROD_WAY label, 0 ord from bf_openway_subproduct "+
	...
	public static String getNextCardRbsNumber(String branch, String client, String acc, boolean is_test_mode)  {
        Connection c = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        Integer pos = 0;
        ...
        try {
                c = ConnectionPool.getConnection();
                //ps = c.prepareStatement("SELECT seq_bf_card_rbsnumber.NEXTVAL id FROM DUAL");
                ps=c.prepareStatement("select a.rbs_number from " +
                  "(select t.* from bf_openway_contract_card t where branch=? and id_client=? and substr(rbs_number, -20)=? and cbs_number is not null order by length(rbs_number) desc, rbs_number desc) a " +
                  " where rownum = 1");
    			ps.setString(1, branch);
    			ps.setString(2, client);
    			ps.setString(3, acc);
                rs = ps.executeQuery();
                if (rs.next()) {
             	   v_tmp=rs.getString("rbs_number");
                } else {
                    //if (is_test_mode) 
                    //    v_tmp="10_C_22618840860000001500";//5_C_22618840860000001500, 8_C_22618840860000001500, 20_C_22618840860000001500
                }

                //String tmp="2_C_22618840860000001500";
                //System.out.println("index of:" + tmp.indexOf("planet")+"!"+tmp.indexOf("_C_"));
                //System.out.println("number:" + tmp.substring(0,tmp.indexOf("_C_")));

                pos=v_tmp.indexOf("_C_");
                if (pos>=1) {
                	v_tmp=v_tmp.substring(0,pos);
                } else {
                	v_tmp="0";
                }
                try 
                {
                	v_tmp =   ((Integer)(Integer.parseInt(v_tmp)+1)).toString();
                }
                catch (Exception e) {
                	v_tmp="1";                	
                }
