...
public class Composer extends GenericForwardComposer {
	private Toolbarbutton btn_back,btn_refresh;
	private Div grd, frm;
	private Grid search_grd;
	private Paging depositListboxPaging;
	private Listbox dataGrid, add_PrivCond$ad_prCond;
	...
	new_depositWnd$btn_add_prCondDep,new_depositWnd$btn_open,add_PrivCond$btn_chk_del;
	...
	//			operationCode, sum, uid);
		...
		ISLogger.getLogger().error("Save_dep: "+com.is.utils.CheckNull.getPstr(e));		
...
/*	private void setParent(Listbox ad_prCond2) {
	...
	if(generalId!=0){
		...
		new ReportBuilder(new SdBook_prn1(cr,cir)).setBranch(branch)
			...
			e.printStackTrace();
		  ISLogger.getLogger().error("btn_sdBook_btn1: "+e.getMessage()+" mess:" +CheckNull.getPstr(e));
		  }
