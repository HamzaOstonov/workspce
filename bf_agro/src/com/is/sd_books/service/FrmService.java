...
public class FrmService {
	...
	public static List<SD_Book> getSdBooksF1(int pageIndex, int pageSize, SD_BookFilter filter, String alias) {
		...
		try {
			...
			for (params = 0; params < flFields.size(); params++) {
				System.out.println("params: " + params + ", pars: " + flFields.get(params).getColobject());
	...
	public static SD_Book getSdBook(HashMap<String, Object> param, Credentials cr) {
		...
		try {
			...
			ps = c.prepareStatement("SELECT * FROM V_SD_BOOKS_ON_ACCOUNT WHERE B_ID = ?");
	...
	public static List<FilterField> getFilterFields(SD_BookFilter filter) {
		...
		if (!CheckNull.isEmpty(filter.getOpen_date())) {
			flfields.add(
					new FilterField(getCond(flfields) + "open_date=to_date(?,'dd.mm.yyyy')", filter.getOpen_date()));
		}
	...
	public static List<RefData> getOperTurn(String code, String alias) {
		...
		try {
			...
			cs = c.prepareCall("{call info.init()}");
			cs.execute();
			ps = c.prepareStatement("SELECT CODE || ' '|| NAME AS LABEL,CODE DATA FROM V_SD_OPER_TURN WHERE DEP = ?");
			ps.setString(1, code);
			rs = ps.executeQuery();
			while (rs.next()) {
	...
	public static List<RefData> getFilials(String alias, int userId) {
		...
		try {
			...
			cs = c.prepareCall("{call info.init()}");
			cs.execute();
			ps = c.prepareStatement("SELECT f.code, f.code id, f.name, "
	...
	public static List<RefData> getPrivCondParms1(String alias, String dep) {
		...
		try {
			...
			cs = c.prepareCall("{call info.init()}");
			cs.execute();
			ps = c.prepareStatement("select ss_depconditions.par AS DATA,ss_deppar.name as LABEL from ss_depconditions "
	...
	public static List<RefData> getSdPrCondValues(String alias, String id, int BookId, String name_Dep) {
		...
		try {
			...
			cs = c.prepareCall("{call info.init()}");
			cs.execute();
			ps = c.prepareStatement("select s.description from ss_deppar s where s.id=? ");
