...
public class AssistantService {
	...
	public static Date getOperDate(String alias){
		...
		try{
			...
			cs.execute();
			cl = c.prepareCall("{? = call info.getDay()}");
	...
	public static String getSavingsBook(long book_id,String alias,String branch){
		...
		try{
			c = ConnectionPool.getConnection(alias);
			ps = c.prepareStatement("SELECT b_ser || ' ' || b_num as name FROM SD_CLERK_ALL "
	...
	public static Date getDeadlineDate(Credentials cr){
		...
		try{
			...
			cs = c.prepareCall("{? = call SD_RECKON.DEADLINE}");
	...
	public static Date getLookBookDate(Credentials cr,String branch,Double book_id){
		...
		try{
			c = ConnectionPool.getConnection(cr.getLogin(),cr.getPassword(),cr.getAlias());
			cs = c.prepareCall("{? = call SD_2010.NEARCAPDATE(?,?)}");
			cs.registerOutParameter(1, java.sql.Types.DATE);
	...
	public static Boolean isDepMonetary(String dep){
		...
		try{
			...
			ps = c.prepareStatement("SELECT CODE_CURRENCY FROM SS_SD A WHERE A.CODE = ?");
			ps.setString(1, dep);
			rs = ps.executeQuery();
			String code = null;
			if (rs.next())
	...
	public static String getUser(){
		...
		try{
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("SELECT USER FROM DUAL");
			rs = ps.executeQuery();
			if (rs.next())
				user = rs.getString(1);
		}
