...
public class ApplDog extends PoiReport{
	...
	public AMedia getRepmd(Map<String, Object> params, Connection c,
		...
		try {
			...
			ISLogger.getLogger().error("Verify_fayl1: "+templf.contains("SD_Dep_sum.doc"));
			
			POIFSFileSystem fs = new POIFSFileSystem(
					new FileInputStream(templf));
			
			ISLogger.getLogger().error("fayl_params2:"+templf);
			ISLogger.getLogger().error("Verify_fayl2: "+templf.contains("SD_Dep_sum.doc"));
				
			HWPFDocument doc = new HWPFDocument(fs);
			infoinit = c.prepareCall("{call info.init()}");
			infoinit.execute();
			...
			ps1=c.prepareStatement("select * from client_j h where h.branch=? and h.id=?");
			...
			if (rs.next()) {
				...
				Date date_close = rs.getDate("DEADLINE");
