...
public class CredOrV extends PoiReport {
	...
	public static CredOrV getInstance() {
		return new CredOrV();
	}
	...
	public AMedia getRepmd(Map<String, Object> params, Connection c,
		...
		try {
			...
			if (rs.next()) {
				...
				fillRow(sheet, 5, 1, ((birthday != null) ? df.format(birthday)
						: emptyString) + "," + birth_place);
				...
				fillRow(sheet, 13, 2, (date_reg != null) ? df.format(date_reg)
						: emptyString);

				Date date_exp = rs.getDate("passport_date_expiration");
				fillRow(sheet, 14, 2, (date_exp != null) ? df.format(date_exp)
						: emptyString);

				String passport_place_registration = rs
						.getString("passport_place_registration");
