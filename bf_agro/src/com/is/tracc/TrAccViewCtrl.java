...
public class TrAccViewCtrl extends GenericForwardComposer{
    ...
    private Toolbar tb;
    private Textbox id,branch,acc_template_id,acc_mfo,account,acc_name, chacc$acc_filter_mask;
    private Textbox aid,abranch,aacc_template_id,aacc_mfo,aaccount,aacc_name ;
    private Textbox fid,fbranch,facc_template_id,facc_mfo,faccount,facc_name,mod_type;
    private Paging traccPaging;
    private  int _pageSize = 14;
    private int _startPageNumber = 0;
    private int _totalSize = 0;
    private boolean _needsTotalSizeUpdate = true;
    private String mbranch, alias, branch1;
    ...
    private GlobuzAccount currentacc = new GlobuzAccount();
    
    
    

    public GlobuzAccount getCurrentacc() {
		return currentacc;
	}
	public void setCurrentacc(GlobuzAccount currentacc) {
...
public void doAfterCompose(Component comp) throws Exception {
   ...
   // }
    ...
    chacc$acc.setItemRenderer(new ListitemRenderer(){
        @SuppressWarnings("unchecked")
        public void render(Listitem row, Object data) throws Exception {
            GlobuzAccount pAccount = (GlobuzAccount) data;
...
public void onDoubleClick$dataGrid$grd() {
            ...
            if ((current!=null) && (current.getAccount() == null)){current.setAccount("");}
           // if(!current.getAcc_mfo().equals(current.getBranch()))
            
            //if((!current.getBranch().equals(current.getAcc_mfo()))&&(mbranch.compareTo(ConnectionPool.getValue("HO", alias))!=0))
            if((current!=null) &&
...
{
	chacc$acc.setModel(new BindingListModelList(TrAccService.getAccount(current, "", alias, acc_mfo.getValue()), false));
...
{
	chacc$acc.setModel(new BindingListModelList(TrAccService.getAccount(current, chacc$acc_filter_mask.getValue(), alias, acc_mfo.getValue()), false));
}
