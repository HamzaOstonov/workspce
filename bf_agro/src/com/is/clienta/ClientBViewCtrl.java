...
public class ClientBViewCtrl extends GenericForwardComposer {
	...
	protected ClientB copyOfCurrent;
	public ClientB current = new ClientB();
	...
	public ClientB getCurrent() {
		return current;
	}

	public void setCurrent(ClientB current) {
		this.current = current;
	}

	public ClientB getCurrentListItem() {
		return currentListItem;
	}

	public void setCurrentListItem(ClientB currentListItem) {
		this.currentListItem = currentListItem;
	}

	public ClientB getNewcl() {
		return newcl;
	}

	public void setNewcl(ClientB newcl) {
		this.newcl = newcl;
	}
	...
	public ClientB currentListItem = new ClientB();
	public ClientB newcl = new ClientB();
	public ClientAFilter filter = new ClientAFilter();
	protected Dao<ClientB> clientDao;
	protected AnnotateDataBinder binder;
	protected String mode;
	protected SearchMode searchMode;
	private static Logger logger = Logger.getLogger(ClientBViewCtrl.class);

	public ClientBViewCtrl() {
		super('$', false, false);
	}
	...
	protected void refreshModel(int activePage, boolean showFirstPage) {// ,
		...
		List<ClientB> clientList = ClientAService.getListWithPaging(activePage, _pageSize, filter, un, pw, alias);
	...
	private void drawActions(ClientB client) {
	...
	public void onDoubleClick$dataGrid$grd() {
		...
		try {
			...
			logger.error("client_b_current: " + currentListItem.getBranch() + " ,client_id: " + currentListItem.getId_client() + ", alias: "+alias);
	...
	private void names(ClientB client) {
	...
	public void onClick$btn_apply$fields_diff_wnd() {
		...
		ClientB fetchedClientFromSap = (ClientB) fields_diff_wnd$diff_grid.getAttribute(ClientUtil.SAP_CLIENT_ATTR);
		ClientB fetchedClientFromEbp = (ClientB) fields_diff_wnd$diff_grid.getAttribute(ClientUtil.EBP_CLIENT_ATTR);
		if (!addgrd.isVisible()) {
	...
	protected void openClient(int action) {
		...
		Validator<ClientB> checkHandler = CheckClient.checkCleanClient(alias, ClientUtil.ACTION_OPEN);
	...
	private void executeAction(int action) {
		...
		if (action == 4 || action == 19) {
			...
			Validator<ClientB> check = CheckClient.checkExisting(alias, action);
	...
	public String toXmlString(ClientB clientA) {
		String xmlString = "";
		/*
	...
	public void compare(ClientB currentCustomer) {
		ClientB fetchedFromSap = null;
		ClientB ebpClient = null;
		/*
