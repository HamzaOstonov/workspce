...
public class ClientAService {
	...
	public Res doAction(ClientB clientA, int actionid) {
		Res res = null;
		...
		try {
			...
			for (Field field : ClientB.class.getDeclaredFields()) {
	...
	private void addToClientP(Connection c, ClientB client) throws SQLException {
		PreparedStatement ps = null;
		try {
	...
	private void addToClientDesc(Connection c, ClientB client) throws SQLException {
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
	...
	private String stubNibbd(Connection c, CallableStatement setParam, ClientB client, int action) throws Exception {
		CallableStatement cs = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
	...
	public List<ClientB> getList(ClientAFilter filter, String alias) {
		Connection c = null;
		List<ClientB> list = new ArrayList<ClientB>();
		PreparedStatement ps = null;
		CallableStatement cs = null;
		ResultSet rs = null;
		int params;
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				list.add(new ClientB(rs.getLong("id"), rs.getString("branch"), rs.getString("id_client"),
						...
						rs.getString("code_country_adr"), null, null, 0, 0, rs.getString("post_address_country")));
			}
		} catch (Exception e) {
			logger.error(CheckNull.getPstr(e));
			e.printStackTrace();
	...
	public static List<ClientB> getListWithPaging(int pageIndex, int pageSize, ClientAFilter filter, String un,
	...
	public ClientB getItemByLongId(String branch, long itemId) {
		throw new UnsupportedOperationException();
	}

	public int update(ClientB ClientA) {
		return 0;
	}

	public int remove(ClientB ClientA) {
		return 0;
	}

	public ClientB create(ClientB ClientA) {
		return null;
	}

	public static ClientB getItemByStringId(String branch, String itemId, String un, String pw, String alias) {
		ClientB customer = null;
		Connection c = null;
		PreparedStatement ps = null;
		CallableStatement cs = null;
		ResultSet rs = null;
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				logger.error("client_b_success: " + branch + " ,client_id: " + itemId + ", alias: "+alias);
				customer = new ClientB();
			...
			logger.error("client_b_error: " + branch + " ,client_id: " + itemId + ", alias: "+alias + CheckNull.getPstr(e));
			e.printStackTrace();
		} finally {
			ConnectionPool.close(c);
			DbUtils.closeStmt(cs);
	...
	public <T1 extends ClientB> void setFilter(T1 filter) {
		this.filter = (ClientAFilter) filter;
	}

	public ClientB create(Connection c, ClientB item) throws Exception {
		return null;
	}

	public int update(Connection c, ClientB item) throws Exception {
		return 0;
	}

	public int remove(Connection c, ClientB item) {
		return 0;
	}

	public ClientB getItemByLongId(Connection c, String branch, long itemId) {
		return null;
	}

	public ClientB getItemByStringId(Connection c, String branch, String itemId) {
		ClientB customer = null;
		PreparedStatement ps = null;
		CallableStatement cs = null;
		ResultSet rs = null;
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				customer = new ClientB(rs.getLong("id"), rs.getString("branch"), rs.getString("id_client"),
						rs.getString("name"), rs.getString("code_country"), rs.getString("code_type"),
						rs.getString("code_resident"), rs.getString("code_subject"), rs.getString("sign_registr"),
						rs.getString("code_form"), rs.getDate("date_open"), rs.getDate("date_close"),
						rs.getString("state"), rs.getInt("kod_err"), rs.getString("file_name"),
