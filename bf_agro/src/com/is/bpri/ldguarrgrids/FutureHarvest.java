...
public class FutureHarvest extends GenericForwardComposer{

	private static final long serialVersionUID = 1L;
	
	private Div futureharvestdiv,grd;
	private Textbox id_client,name,massiv,street,home,home_num,doc_num,inn_reestr,notarial_doc_numb,
	inn,name2,sowing_area;
	private Datebox doc_date,start_date,acomp_date;
	private RefCBox region_id,distr_id,currency,klass_o,code_subject,niki_res1,acomp_name,acomp_curr;
	private Decimalbox acomp_summa,amount;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		refCboxsetModels();
		inputDatas();
		futureharvestdiv.setVisible(true);
	...
	private void inputDatas(){
		if(clickbtn.equals("double")){
			...
			sowing_area.setValue(current.getSowing_area());
		} else if(clickbtn.equals("add")){
			Utils.clearForm(futureharvestdiv);
			futureharvestdiv.setVisible(false);
		}
	...
	public void onClick$btn_saving(){
		try {
			...
			current.setSowing_area(sowing_area.getValue());
