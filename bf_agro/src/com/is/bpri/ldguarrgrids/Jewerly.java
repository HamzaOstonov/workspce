...
public class Jewerly extends GenericForwardComposer{
	
	private Div jewerlydiv,grd;
	private Textbox id_client,name,massiv,street,home,home_num,doc_num;
	private RefCBox region_id,distr_id,currency,klass_o,acomp_name,acomp_curr;
	private Decimalbox acomp_summa,amount;
	private Doublebox massa;
	private Datebox doc_date,acomp_date;
	...
	private void inputDatas(){
		if(clickbtn.equals("double")){
			...
			massa.setValue(current.getMassa());
		} else if(clickbtn.equals("add")){
			Utils.clearForm(jewerlydiv);
			jewerlydiv.setVisible(false);
		}
	...
	public void onClick$btn_saving(){
		try {
			...
			current.setMassa(massa.getValue());
