...
public class Statesecurities extends GenericForwardComposer{

	private static final long serialVersionUID = 1L;

	private Div statesecuritiesdiv,grd;
	private Textbox id_client,name,address,doc_num,inn_reestr,inn,mfo,account,stock_nominal_value,
				stock_count,stock_diskont,stock_name,name2,niki_inn,depositary,depositary_account;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		refCboxsetModels();
		inputDatas();
		statesecuritiesdiv.setVisible(true);
		if(guar_id.equals("36")){
			guarrow.setVisible(false);
		} else {
			guarrow.setVisible(true);
		}
	...
	private void inputDatas(){
		if(clickbtn.equals("double")){
			...
			depositary.setValue(current.getDepositary());
			depositary_account.setValue(current.getDepositary_account());
			start_date.setValue(current.getStart_date());
		} else if(clickbtn.equals("add")){
			Utils.clearForm(statesecuritiesdiv);
			statesecuritiesdiv.setVisible(false);
		}
	...
	public void onClick$btn_saving(){
		try {
			...
			current.setDepositary(depositary.getValue());
			current.setDepositary_account(depositary_account.getValue());
