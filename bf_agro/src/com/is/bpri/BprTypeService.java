...
public class BprTypeService {
	...
	public static void create(BprType bprtype, String alias,Res res) {
		...
		try {
			...
			res.setCode(1);
			ISLogger.getLogger().error("À âîò òóò îøèáêà");
	...
	public static List<RefData> getCurrency(String branch){ // ---------- Âàëþòû
		if (Currency_ == null) {
	...
	public static List<RefData> getDeal_id(String branch){ // ---------- Âàëþòû
		if (Deal_id_ == null) {
	...
	public static List<RefData> getBprType(String branch){ // ---------- Âàëþòû
		return Utils.getRefData("select s.id data, s.id||'-'||s.name label from ss_deal s where s.group_id=193 ",branch);
	}
	
	public static List<RefData> getBprTypeState(String branch){ // ---------- Âàëþòû
	...
	protected static List<RefData> getBranchs(String branch){
		return Utils.getRefData("select '00' bank_id, 'ÂÑÅ ÔÈËÈÀËÛ' bank_id from dual union all select bank_id,bank_id from s_mfo where bank_type='"+getBank_type(branch)+"' and act='A'", "");
	}
	
	protected static List<RefData> getRegions(){
		return Utils.getRefData("select '00' region_id, 'ÂÑÅ ÐÅÃÈÎÍÛ' region_nam from dual union all select region_id,region_nam from s_region where act='A'", "");
	}
	...
	protected static List<RefData> getTarget_clients(){
		return Utils.getRefData("select 'P' id,'Äëÿ ôèç. ëèö' from dual union all select 'J' id,'Äëÿ þð. ëèö' from dual", "");
	}
