...
//		crm.setVisible(true);
	...
	public void onClick$btn_cleardper(){
		...
		aclient_i.setConstraint("");aclient_i.setValue("");
		aclient_i2.setConstraint("");aclient_i2.setValue("");
		aclient_i3.setConstraint("");aclient_i3.setValue("");
		...
		acc_dep.setValue("");
		//out_tmp.setValue(new BigDecimal(""));
		name_acc.setValue("");
		atotalsum.setValue("");
		amtcn.setConstraint("");amtcn.setValue("");
		asumma.setConstraint("");;asumma.setValue(new BigDecimal(""));
		asumma2.setValue("");
		asumma4.setValue("");
		acentsumma.setValue("");
		acentsumma2.setValue("");
		akurs_val.setValue("");
		asumma3.setValue("");
		asumma5.setValue("");
		aprofit.setValue("");
...
//		String eval = aeval.getSelectedItem().getValue().toString();
		...
		String info_id = dper_infoService.getId(alias);
		System.out.println("info_id = "+info_id);
		renderDper_books(info_id);
		
		
		HandleOperationRequest h = KoronaPayService.getHadleOperationByUIN(amtcn.getValue());
		String[] fullName = h.getSenderFullName().split(" ");
		String[] fullName2 = h.getReceiverFullName().split("");
		...
		try {
	        ...
	        map.put("uin", h.getuIN());
	        map.put("operation", String.valueOf(h.getOperation()));
	        map.put("nciDocId", info_id);
	        map.put("nciDocDate", new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ssXXX").format(new Date()));
	        try {
	            map.put("payerOtherName", fullName[2]);
	            } catch (Exception e) {
	               map.put("payerOtherName", "");
	            } 
	            
	            map.put("payeeLastName", fullName2[0]);
	            map.put("payeeFirstName", fullName2[1]);
	            
	            try {
	            map.put("payeeOtherName", fullName2[2]);
	            } catch (Exception e) {
	               map.put("payeeOtherName", "");
	            }
	        map.put("absClientId", clientId);
	        map.put("amount", String.valueOf(h.getPayAmount()));
	        map.put("exp", String.valueOf(h.getPayExp()));
	        map.put("cur", String.valueOf(h.getPayCur()));
	      ...
	      e.printStackTrace();
	      alert ("Сервер Золотая Корона не отвечает"
	      		+"Ошибка: WS Translator не отвечает"
	...
	// MTCN код
	public void onOK$amtcn(Event event){
		
		setData(amtcn.getValue());
		
	}
	
	// MTCN код
		public void onChange$amtcn(Event event){
					
			newdper.setMtcn(amtcn.getValue()); 
			aveoper.setSelecteditem("42216");
			au1f2.setSelecteditem("2");
			acurrency.setSelecteditem("840");
			aeval.setSelecteditem("840");
	...
	protected void setDataInNewdper(){
		...
		System.out.println("outTmp "+out_tmp.getValue());
		System.out.println("nameAcc "+name_acc.getValue());
	...
	private void setData(String uin) {
		System.out.println(newdper.toString());
	    HandleOperationRequest rho = KoronaPayService.getHadleOperationByUIN(uin);
	      BigDecimal bd = new BigDecimal("1");
	      
	      for(int i=0; i<akind.getItems().size(); i++) {
	        if(akind.getItemAtIndex(i).getValue().equals(rho.getOperation()+"")) {
	          akind.setValue(akind.getItemAtIndex(i).getLabel());
	          asumma4.setValue(new BigDecimal(rho.getSenderCommissionAmount()));
	          aprofit.setValue(new BigDecimal(SumsService.calcPercent(newdper, alias)));
	        }
	        
	      }
	      
	      System.out.println("rho.getPayAmount()  ==  "+rho.getPayAmount());
	      
	      
	      akind.setValue(akind.getItemAtIndex(rho.getOperation()).getLabel()+"");
	      onChange$akind();
	      ...
	      String strs = KoronaPayDBHelper.countries(rho.getFromCountryIso());
	      for (RefData s : countries) {
	        if (s.getLabel().equals(strs)) {
	          astrs.setSelectedIndex(countries.indexOf(s));
	        }
	      }

	      String strr = KoronaPayDBHelper.countries(rho.getToCountryIso());
	      for (RefData s : countries) {
	        if (s.getLabel().equals(strr)) {
	          astrr.setSelectedIndex(countries.indexOf(s));
	        }
	      }
	      
	      setGroup_id(astrr.getValue());

	      for (RefData s : dperKinds) {
	        if (s.getData().equals(dperKinds)) {
	        }
	      }
	      ...
	      if(rho.getOperation()==1) {
		     fullName = rho.getSenderFullName().split(" ");  
		     
		     
		     
		     aclient_i9.setValue(rho.getSenderDocNumber());
		     aclient_i8.setValue(rho.getSenderDocSeries());

		     aclient_i10.setValue(rho.getSenderRegAddress());
	         aclient_i12.setValue(rho.getSenderBirthCity());
		     
		     
		      try {
		        aclient_i11date.setValue(new SimpleDateFormat("dd.MM.yyyy").parse(rho.getSenderBirthDate()));
		      } catch (Exception e) {
		        e.printStackTrace();
		      }

		      country = KoronaPayDBHelper.countries(rho.getSenderCitizenship());
		      System.out.println(country);
		      
		      for (RefData s : countries) {
		        if (s.getLabel().equals(country)) {
		          aclient_i13code_str.setSelectedIndex(countries.indexOf(s));
		        }
		      }
		     
		     
	      }else if(rho.getOperation()==0) {
	    	  fullName = rho.getReceiverFullName().split(" ");
	    	  
	    	  /* aclient_i9.setValue(rho.getReceiverDocNumber());
			  aclient_i8.setValue(rho.getReceiverDocSeries());

		      aclient_i10.setValue(rho.getReceiverRegAddress());
	          aclient_i12.setValue(rho.getReceiverBirthCity());
	    	  
	          try {
			        aclient_i11date.setValue(new SimpleDateFormat("dd.MM.yyyy").parse(rho.getReceiverBirthDate()));
			      } catch (Exception e) {
			        e.printStackTrace();
			      }
                
			      country = KoronaPayDBHelper.countries(rho.getReceiverCitizenship());
			      System.out.println(country);
	      ...
	      aclient_i.setValue(fullName[0]);
	      aclient_i2.setValue(fullName[1]);
	      aclient_i3.setValue(fullName[2]);

	      
	      BigDecimal sum = asumma.getValue();
			sum.add(asumma4.getValue());
			asumma5.setValue(sum);
