...
import com.is.file_reciever.energo.Ext_created_object;
import com.is.file_reciever.energo.Ext_in_file_records;
import com.is.file_reciever.energo.Ext_in_file_recordsFilter;
import com.is.utils.CheckNull;
import com.is.utils.RefCBox;
import com.is.utils.RefData;

public class Tieto_transactions_vc extends GenericForwardComposer{
    private Div frm;
...
public void doAfterCompose(Component comp) throws Exception {
    ...
    List<RefData> ext_files = Tieto_transactions_service.getExt_files(current_file_fr_id);
...
public void onDoubleClick$dataGrid$grd() {
            ...
            parsed_data.setValue(Tieto_transactions_service.get_file_records_details(current.getId()));
            
            Tieto_transactions_service.get_file_deal_id_from_file(current.getIn_file_id());
          ...
          public void onEvent(Event e)
             ...
             //if (Messagebox.ON_YES.equals(e.getName()))
        	  if (answer == Messagebox.YES)
                {
            	 Tieto_transactions_service.remove_file(Long.parseLong(cmb_ext_file.getValue()));
            	 	refreshModel(_startPageNumber);
                }
             else
...
public void onSelect$cmb_ext_file()
{
    ...
    btn_remove_file.setVisible(
    		Tieto_transactions_service.get_ext_file_state(Long.parseLong(cmb_ext_file.getValue()))
    		== 0
    		);
    
    if (Tieto_transactions_service.get_ext_file_state(Long.parseLong(cmb_ext_file.getValue())) == 2l)
    	doc_action.setLabel("Ввести проводки");
    if (Tieto_transactions_service.get_ext_file_state(Long.parseLong(cmb_ext_file.getValue())) == 3l)
    	doc_action.setLabel("Утвердить проводки");
    if (Tieto_transactions_service.get_ext_file_state(Long.parseLong(cmb_ext_file.getValue())) == 4l)
...
{
	List<Ext_created_object> created_objects = Tieto_transactions_service.
	...
	for(Ext_created_object current_object : created_objects)
	{
		...
		cur_li.addEventListener(Events.ON_DOUBLE_CLICK, new EventListener()
    	{
					...
					throws Exception 
			{
				...
				String show_object_url = Tieto_transactions_service.get_module(
		...
		Listcell lc = new Listcell(
				Tieto_transactions_service.get_deal_group_name(current_object.getDeal_group_id())
				);
...
public void onClick$doc_action() throws NumberFormatException, Exception
{
	...
	try
	{
		...
		Tieto_transactions_service.input_documents(c, Long.parseLong(cmb_ext_file.getValue()));
		alert("Документы введены");
		Tieto_transactions_service.fill_02_005_pre(Long.parseLong(cmb_ext_file.getValue()), c);
		alert("Заполнены таблицы отчета iias 02 005");
		ps_update = c.prepareStatement("update ext_in_files t set t.state_id = 5 where t.id = ?");
		ps_update.setLong(1, Long.parseLong(cmb_ext_file.getValue()));
		ps_update.execute();
...
public void onClick$doc_action_remove() throws Exception
{
	...
	{
		c = ConnectionPool.getConnection(un, pwd);
		Tieto_transactions_service.documents_action(Long.parseLong(cmb_ext_file.getValue()), 6l, c);
		c.commit();
	}
	catch(Exception e)
	{
...
public void onClick$fill_02_005() throws NumberFormatException, Exception
{
	Connection c = null;
	try
	{
