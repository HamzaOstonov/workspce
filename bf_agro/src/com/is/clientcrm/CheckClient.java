...
public class CheckClient extends Validator<ClientA> {
	...
	private boolean checkObjectiveData() {
		...
		if ( client.getJ_date_registration().compareTo(getOperDay())>0 ) {
			message = "Дата регистрации не может быть больше банковской даты";
			return false;
		}
	...
	private boolean checkJur() {
		//shuerga tekshirish quymaizmi
		//yur liso imeyushiy osnovb shet v dr banke da
		//unikal kod majburiyligi va 8 ta raqamligini tekshirish kerak
        if (client.getSign_registr().equals("3") && isOrdinaryJur()){
    		if (CheckNull.isEmpty(client.getId_client())) {
    			message = "Введите Код клиента";
    			return false;
    		}
    		if (client.getId_client()!=null && client.getId_client().length() != 8) {
    			message = "Длина поля Код клиента должна быть равна 8 символов";
    			return false;
    		}
        }
	...
	private boolean checkIp() {
		if (CheckNull.isEmpty(client.getP_pinfl())) {
			message = "Введите ПИНФЛ";
			return false;
		}
		if (client.getP_pinfl().length()!=14) {
			message = "Длина ПИНФЛ должень быть 14 знак и содержать только цифры";
			return false;
		}
		...
		if (CheckNull.isEmpty(client.getJ_type_activity())) {
			message = "Введите вид деятельности";
			return false;
		}
		if (CheckNull.isEmpty(client.getJ_opf())) {
			message = "Введите корректный код организационно-правовой формы.";
			return false;
		}
		...
		if (CheckNull.isEmpty(client.getJ_director_name())) {
			message = "Введите владелец патента";
			return false;
		}
		...
		try {
			fdate = new SimpleDateFormat("dd.MM.yyyy").parse("01.01.1910");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("fdate and client.getP_passport_date_registration()" + client.getP_passport_date_registration() + " " + fdate);
		if (client.getP_passport_date_registration().before(fdate)) {
			System.out.println("fdate and client.getP_passport_date_registration()" + client.getP_passport_date_registration() + " " + fdate);

			message = "Дата выдачи паспорта не должна быть раньше 01.01.1992 годаЫЫЫ";
			return false;
		}
		if (client.getP_passport_date_registration().after(getOperDay())) {
		...
		/*if (!ignoreDateValidation && client.getP_type_document().equals("6")) {
			Date dateReg = client.getP_passport_date_registration();
			Date dateExp = client.getP_passport_date_expiration();
            ...
            if (dateReg.compareTo(dateExp) != 0) {
				message = "Дата действия паспорта должнa быть: от даты рождения 10 лет минус 1 день (Биометрический паспорт)";
				return false;
			}
		}
		if (!ignoreDateValidation && client.getP_type_document().equals("1")) {
			Date dateReg = client.getP_passport_date_registration();
			Date dateExp = client.getP_birthday();
			Calendar c = Calendar.getInstance();
			...
			if (dateReg.compareTo(dateExp) != 0) {
				message = "Дата действия паспорта должнa быть: от даты рождения 25 лет минус 1 день (старый паспорт)";
				return false;
			}
		}*/

		if (client.getP_passport_date_expiration().before(getOperDay())) {
			message = "Дата действия паспорта не должнa быть меньше даты операционного дня";
			return false;
		}
		
		Date TOdate = null;
		try {
			TOdate = new SimpleDateFormat("dd.MM.yyyy").parse("01.01.1910");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();			
		}
		
		if (client.getP_birthday().before(TOdate)) {
			message = "Дата рождения не должна быть раньше 01.01.1910 года";
			return false;
		}
		if (client.getP_birthday().after(getOperDay())) {
	...
	/*	if (CheckNull.isEmpty(client.getP_code_tax_org())) {
		...
		Date gdate = null;
		try {
			gdate = new SimpleDateFormat("dd.MM.yyyy").parse("01.01.1992");
		} catch (ParseException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		if (client.getP_passport_date_registration().before(gdate)) {
			message = "Дата выдачи паспорта не должна быть раньше 01.01.1992 годаAS";
