...
public class ClientAService {
	...
	private int count;
	private static String v_const_1001;
	...
	public Res doAction(ClientA clientA, int actionid) {
		...
		try {
			...
			for (Field field : ClientA.class.getDeclaredFields()) {
				...
				//System.out.println(field.getName().toUpperCase()+"                                               aaa");
				cs.execute();
			}
			
			cs.setString(1, "P_NIBBD_REG_TYPE");
			cs.setString(2, "4");
			cs.execute();
			...
			/*
			--открыть клиента и основной счет  

			begin
			      param.setparam('N_BRANCH', '00394');  --мфо
			      param.setparam('N_QUERY_NO', '1'); --nomer zaprosa 
				  param.setparam('N_NAME', '');  --imya klienta
			      param.setParam('N_CODE_RESIDENT', 'Д'); --ResidentFlag, Д-резидент, Н-нерезидент
			      param.setParam('N_CODE_COUNTRY', '860');  --код страны
			      param.setParam('N_NUMBER_REGISTRATION_DOC', ''); -- номер регистрации 
				  param.setParam('N_PLACE_REGIST_NAME', '');  -- имя органа который зарегистрировал клиента
				  param.setParam('N_DATE_REGISTRATION', '');  --дата регистрации
				  param.setParam('N_INN', '');  --ИНН
				  param.setParam('N_CODE_TYPE', '');  --тип клиента (ЧТК, БДУ, ЧЛП и т.д.)
				  param.setParam('N_CODE_HEAD_ORGANIZATION', ''); --код головной предприятии, если нету то 0
				  param.setParam('N_INN_HEAD_ORGANIZATION', '');  -- ИНН головной предприятии, если нету то 0
				  param.setParam('N_OPF', ''); --код организационно правовая форма
				  param.setParam('N_CODE_FORM', ''); --форма собственности
			      param.setParam('N_SOATO', '');  --код адреса
			      param.setParam('N_OKPO', '');   --код юридического лица (ОКПО)
				  param.setParam('N_CODE_SECTOR', '');  --код отрасли
			      param.setParam('N_ACC_BAL', '');	 -- код балансовый счет		
			      param.setParam('N_CODE_CURRENCY', '');  --Код валюты
				  param.setParam('N_ACC_ID_ORDER', '');  --Порядковый номер счёта
				  param.setParam('N_REGION', '');  --Код области
				  param.setParam('N_DISTR', '');   --Код района
			      
				  param.setparam('N_PARENT_ID', '');  -- pole ID iz tablisa CLIENT
			  
			      kernel.doaction(220, 1, 12); --создать и отправить

			end;
			*/
			   
			if (actionid == ACTION_CONFIRM && clientA.getSign_registr().equals("1")) {
			//if (true) {
				//nibbd 1-zapros
				cs.setString(1, "N_BRANCH");
				cs.setString(2, clientA.getBranch());
				cs.execute();

				cs.setString(1, "N_QUERY_NO");
				cs.setString(2, "1");
				cs.execute();

				cs.setString(1, "N_NAME");
				cs.setString(2, clientA.getName());
				cs.execute();

				cs.setString(1, "N_CODE_RESIDENT"); 
				cs.setString(2, clientA.getCode_resident().equals("1") ? "Д" : "Н" ); 
				cs.execute();

				cs.setString(1, "N_CODE_COUNTRY");
				cs.setString(2, clientA.getCode_country());
				cs.execute();

				cs.setString(1, "N_NUMBER_REGISTRATION_DOC");
				cs.setString(2, clientA.getJ_number_registration_doc());
				cs.execute();

				cs.setString(1, "N_PLACE_REGIST_NAME");
				cs.setString(2, clientA.getJ_place_regist_name());
				cs.execute();

				cs.setString(1, "N_DATE_REGISTRATION");
				cs.setDate(2, Util.sqlDate((Date) clientA.getJ_date_registration()));
				cs.execute();

				cs.setString(1, "N_INN");
				if (clientA.getP_pinfl()==null || clientA.getP_pinfl().equals(""))
				  cs.setString(2, clientA.getJ_number_tax_registration());
				else
					cs.setString(2, clientA.getP_pinfl());
				cs.execute();

				HashMap<String, String> clientCodeLettersMap = DictionaryKeeper.getClientCodeLetters();
				cs.setString(1, "N_CODE_TYPE");
				cs.setString(2, clientCodeLettersMap.get(clientA.getCode_type()));
				cs.execute();

				cs.setString(1, "N_CODE_HEAD_ORGANIZATION");
				cs.setString(2, clientA.getJ_code_head_organization() );
				cs.execute();

				cs.setString(1, "N_INN_HEAD_ORGANIZATION");
				cs.setString(2, clientA.getJ_inn_head_organization());
				cs.execute();

				cs.setString(1, "N_OPF");
				cs.setString(2, clientA.getJ_opf().trim());
				cs.execute();

				cs.setString(1, "N_CODE_FORM");
				cs.setString(2, clientA.getCode_form());
				cs.execute();

				cs.setString(1, "N_SOATO");
				cs.setString(2, clientA.getJ_soato());
				cs.execute();

				cs.setString(1, "N_OKPO");
				cs.setString(2, clientA.getJ_okpo());
				cs.execute();
				
				cs.setString(1, "N_CODE_SECTOR");
				cs.setString(2, clientA.getJ_code_sector());
				cs.execute();

				cs.setString(1, "N_ACC_BAL");
				cs.setString(2, clientA.getJ_nibbd_acc_bal());
				cs.execute();

				cs.setString(1, "N_CODE_CURRENCY");
				cs.setString(2, "000");
				cs.execute();

				cs.setString(1, "N_ACC_ID_ORDER");
				cs.setString(2, clientA.getJ_nibbd_acc_id_order());
				cs.execute();

				cs.setString(1, "N_REGION");
				cs.setString(2, clientA.getJ_region());
				cs.execute();

				cs.setString(1, "N_DISTR");
				cs.setString(2, clientA.getJ_distr());
				cs.execute();

				cs.setString(1, "N_PARENT_ID");
				cs.setString(2, ""+clientA.getId());
				cs.execute();

				acs.setInt(1, 220);
				acs.setInt(2, 1); //nomer zaprosa
				acs.setInt(3, 12);
				acs.execute();
			}

			if (actionid == ACTION_CHANGE_NIBBD && clientA.getSign_registr().equals("1")) {

			      /*--формирование 5-запроса в НИББД
			      param.setparam('N_ID_CLIENT' ,rec.ID_CLIENT              );	  
			      param.setparam('N_BRANCH',rec.branch);
			      param.setparam('N_NAME' ,rec.NAME                   );
			      param.setparam('N_CODE_RESIDENT' ,rec.CODE_RESIDENT          );
			      param.setparam('N_CODE_COUNTRY' ,rec.CODE_COUNTRY           );
			      param.setparam('N_NUMBER_REGISTRATION_DOC' ,rj.NUMBER_REGISTRATION_DOC);
			      param.setparam('N_PLACE_REGIST_NAME' ,rj.PLACE_REGIST_NAME      );
			      param.setparam('N_DATE_REGISTRATION' ,rj.DATE_REGISTRATION      );
			      param.setparam('N_INN' ,rj.number_tax_registration                    );
			      param.setparam('N_CODE_TYPE' ,rec.CODE_TYPE              );
			      param.setparam('N_CODE_HEAD_ORGANIZATION' ,rj.CODE_HEAD_ORGANIZATION );
			      param.setparam('N_INN_HEAD_ORGANIZATION' ,rj.INN_HEAD_ORGANIZATION  );
			      param.setparam('N_OPF' ,rec.OPF                    );
			      param.setparam('N_CODE_FORM' ,rec.CODE_FORM              );
			      param.setparam('N_SOATO' ,rj.SOATO                  );
			      param.setparam('N_OKPO' , rj.okpo              );
			      param.setparam('N_CODE_SECTOR' ,rj.CODE_SECTOR            );
			      param.setparam('N_REASON' ,'корректировка с-но справочника N93'       );
			      param.setparam('N_REGION' ,rj.REGION                 );
			      param.setparam('N_DISTR' ,rj.DISTR                  );
				  param.setparam('N_PARENT_ID', rec.id);
				  if nvl(rec.subbranch,rec.branch)!=rec.branch then
				    param.setparam('N_SUBBRANCH' , rec.subbranch );
				  end if;
				  
			      kernel.doaction(220, 5, 12); --создать и отправить
                  */
				cs.setString(1, "N_ID_CLIENT");
				cs.setString(2, ""+clientA.getId_client());
				cs.execute();
				...
				cs.setString(1, "N_REASON");
				cs.setString(2, clientA.getJ_nibbd_reason());
				cs.execute();
				...
				if (clientA.getSubbranch()!=null && !clientA.getSubbranch().equals("") && !clientA.getSubbranch().equals(clientA.getBranch())){
					cs.setString(1, "N_SUBBRANCH");
					cs.setString(2, clientA.getSubbranch());
					cs.execute();					
				}			    					
				acs.setInt(1, 220);
				acs.setInt(2, 5); //zapros raqami(turi)
				acs.setInt(3, 12);
				acs.execute();
			}
	...
	public static List<FilterField> getFilterFields(ClientAFilter filter) {
		...
		if (!CheckNull.isEmpty(filter.getJ_opf())) {
			flfields.add(new FilterField(DbUtils.getCond(flfields) + "trim(j_opf)=?", filter.getJ_opf().trim()));
		}
		...
		try {
			...
			while (rs.next()) {
				list.add(new ClientA(rs.getLong("id"), rs.getString("branch"), rs.getString("id_client"),
						...
						rs.getString("j_director_family"),
						rs.getString("j_director_first_name"),
						rs.getString("j_director_patronymic"),
						rs.getString("j_director_drv_permit_ser"),
						rs.getString("j_director_drv_permit_num"),
						rs.getDate("j_director_drv_permit_reg_d"),
						rs.getDate("j_director_drv_permit_exp_d"),
						rs.getString("j_director_drv_permit_place"),
						...
						rs.getString("j_accountant_family"),
						rs.getString("j_accountant_first_name"),
						rs.getString("j_accountant_patronymic"),
						rs.getString("j_accountant_drv_permit_ser"),
						rs.getString("j_accountant_drv_permit_num"),
						rs.getDate("j_accountant_drv_permit_reg_d"),
						rs.getDate("j_accountant_drv_permit_exp_d"),
						rs.getString("j_accountant_drv_permit_place"),						
						
						
						rs.getLong("j_327"), rs.getDate("j_patent_expiration"),
						...
						rs.getString("p_drive_permit_ser"), rs.getString("p_drive_permit_num"), rs.getDate("p_drive_permit_reg_d"),
						rs.getDate("p_drive_permit_exp_d"),  rs.getString("p_drive_permit_place")
				));
   ...
   public static ClientA getItemByPassport(String branch, String p_ser, String p_num, String un, String pw, String alias) {
	        ...
	        try {
	            ...
	            cs = c.prepareCall("{call info.init()}");
	            cs.execute();
	            ps = c.prepareStatement("select * from v_client t where t.branch = ? and t.p_passport_serial = ? and t.p_passport_number= ?");
	            ...
	            if (rs.next()) {
	                ...
	                customer.setJ_opf(rs.getString("j_opf").trim());
	                ...
	                customer.setJ_code_head_organization((customer.getJ_code_head_organization() != null) ? customer.getJ_code_head_organization().trim() : "0");
	                customer.setJ_inn_head_organization((customer.getJ_inn_head_organization() != null) ? customer.getJ_inn_head_organization().trim() : "0");
	            }
	        }
	        catch (Exception e) {
	...
	public static ClientA getItemByStringId(String branch, String itemId, String un, String pw, String alias) {
		...
		try {
			...
			if (rs.next()) {
				...
				customer.setJ_director_birth_place(rs.getString("j_director_birth_place"));
				customer.setJ_director_address(rs.getString("j_director_address"));
				customer.setJ_director_family          (rs.getString("j_director_family"));
				customer.setJ_director_first_name      (rs.getString("j_director_first_name"));
				customer.setJ_director_patronymic      (rs.getString("j_director_patronymic"));
				customer.setJ_director_drv_permit_ser  (rs.getString("j_director_drv_permit_ser"));
				customer.setJ_director_drv_permit_num  (rs.getString("j_director_drv_permit_num"));
				customer.setJ_director_drv_permit_reg_d(rs.getDate("j_director_drv_permit_reg_d"));
				customer.setJ_director_drv_permit_exp_d(rs.getDate("j_director_drv_permit_exp_d"));
				customer.setJ_director_drv_permit_place(rs.getString("j_director_drv_permit_place"));
				...
				customer.setJ_accountant_birth_place(rs.getString("j_accountant_birth_place"));
				customer.setJ_accountant_address(rs.getString("j_accountant_address"));
				customer.setJ_accountant_family          (rs.getString("j_accountant_family"));
				customer.setJ_accountant_first_name      (rs.getString("j_accountant_first_name"));
				customer.setJ_accountant_patronymic      (rs.getString("j_accountant_patronymic"));
				customer.setJ_accountant_drv_permit_ser  (rs.getString("j_accountant_drv_permit_ser"));
				customer.setJ_accountant_drv_permit_num  (rs.getString("j_accountant_drv_permit_num"));
				customer.setJ_accountant_drv_permit_reg_d(rs.getDate("j_accountant_drv_permit_reg_d"));
				customer.setJ_accountant_drv_permit_exp_d(rs.getDate("j_accountant_drv_permit_exp_d"));
				customer.setJ_accountant_drv_permit_place(rs.getString("j_accountant_drv_permit_place"));
	...
	// CRM uchun
	public static ClientA getClientAByStringId(String branch, String itemId, String un, String pw, String alias) {
	...
	public static ClientA getEBPData_from_table(String branch, String p_inn, String p_id, String p_query_type) {
        ClientA customer = new ClientA();
        Connection c = null;
        PreparedStatement ps = null;
        CallableStatement cs = null;
        ResultSet rs = null;
        try {
            ...
            if (p_query_type.equals("3")) {
            	//select * from SSI_INDIVIDUAL_DETAILS where id=:p_id and (branch=:p_branch or branch is null) and (tin=:p_inn or pinfl=:p_inn) and rownum=1
            	ps = c.prepareStatement("select * from SSI_INDIVIDUAL_DETAILS where id=? and (branch=? or branch is null) and (tin=? or pinfl=?) and rownum=1");
                ps.setString(1, p_id);
                ps.setString(2, branch);
                ps.setString(3, p_inn);
                ps.setString(4, p_inn);
                rs = ps.executeQuery();
                if (rs.next()) {
                	customer.setJ_number_tax_registration (rs.getString("TIN"));
                	customer.setP_inps(rs.getString("PINFL"));
                	customer.setP_pinfl(rs.getString("PINFL"));
                	customer.setCode_resident(rs.getString("RESIDENT_CODE"));
                	customer.setCode_country(rs.getString("COUNTRY_CODE"));
                	customer.setP_code_gender(rs.getString("GENDER"));
                	customer.setP_birthday(rs.getDate("BIRTH_DATE"));
                	customer.setP_birth_place(rs.getString("BIRTH_PLACE"));
                	customer.setName(rs.getString("LASTNAME")+" "+rs.getString("FIRSTNAME")+" "+rs.getString("MIDDLENAME"));
                	customer.setP_code_adr_region(rs.getString("ADDRESS_REGION"));
                	customer.setP_code_adr_distr(rs.getString("ADDRESS_SUBREGION"));
                	customer.setP_post_address(rs.getString("ADDRESS"));
                	customer.setJ_phone(rs.getString("PHONE"));
                	customer.setJ_fax(rs.getString("FAX"));
                	customer.setJ_email(rs.getString("EMAIL"));
                	customer.setP_phone_mobile(rs.getString("MOBILE_PHONE"));
                	customer.setJ_code_tax_org(rs.getString("TAX_ORGANIZATION_CODE"));
                	customer.setJ_date_registration(rs.getDate("REGISTRATION_DATE"));
                	if (rs.getDate("REGISTRATION_EXPIRE_DATE")!=null)
                		customer.setJ_patent_expiration(rs.getDate("REGISTRATION_EXPIRE_DATE"));
                	customer.setJ_place_regist_name(rs.getString("REGISTRATION_ISSUER"));
                	customer.setJ_number_registration_doc(rs.getString("REGISTRATION_DOC_NUMBER"));
                	customer.setCode_form(rs.getString("PROPERTY_FORM"));
                	customer.setJ_opf(rs.getString("LEGAL_FORM"));
                	customer.setJ_region(rs.getString("ACTIVITY_REGION"));
                	customer.setJ_distr(rs.getString("ACTIVITY_SUBREGION"));
                	customer.setJ_post_address(rs.getString("ACTIVITY_ADDRESS"));
                	customer.setJ_director_name(rs.getString("LASTNAME")+" "+rs.getString("FIRSTNAME")+" "+rs.getString("MIDDLENAME"));
                	if (rs.getDate("ACTIVITY_TYPE")!=null)
                		customer.setJ_type_activity(rs.getString("ACTIVITY_TYPE"));
                } else {
                	customer.setSign_100("no data found in SSI_INDIVIDUAL_DETAILS");
                }

            } else {
            	//select * from SSI_LEGAL_ENTITY where id=:p_id and branch=:p_branch and tin=:p_inn and rownum=1
            	ps = c.prepareStatement("select * from SSI_LEGAL_ENTITY where id=? and branch=? and tin=? and rownum=1");
                ps.setString(1, p_id);
                ps.setString(2, branch);
                ps.setString(3, p_inn);
                rs = ps.executeQuery();
                if (rs.next()) {
                    ...
                    customer.setJ_number_tax_registration (rs.getString("TIN"));
                    customer.setName (rs.getString("NAME"));
                    customer.setJ_short_name (rs.getString("NAME_SHORT"));
                    customer.setJ_region(rs.getString("ADDRESS_REGION"));
                    customer.setJ_distr(rs.getString("ADDRESS_SUBREGION"));
                    customer.setJ_soato (rs.getString("ADDRESS_CODE"));
                    customer.setJ_post_address(rs.getString("ADDRESS"));
                   	customer.setJ_phone(rs.getString("PHONE"));
                  	customer.setJ_fax(rs.getString("FAX"));
                  	customer.setJ_email(rs.getString("EMAIL"));
  				    customer.setJ_code_tax_org(rs.getString("TAX_ORGANIZATION_CODE"));                 
  					...
  					customer.setJ_place_regist_name(rs.getString("REGISTRATION_ISSUER"));                  
  					customer.setJ_number_registration_doc(rs.getString("REGISTRATION_DOCUMENT"));                  
  					customer.setCode_form(rs.getString("PROPERTY_FORM"));
                    customer.setJ_opf(rs.getString("LEGAL_FORM"));
                    customer.setJ_code_sector (rs.getString("OKED"));                    
                    customer.setJ_okpo (rs.getString("JURIDICAL_FORM"));                    
                    customer.setJ_inn_head_organization (rs.getString("HEADER_TIN"));                    
                    customer.setJ_director_name (rs.getString("NAME_DIRECTOR"));                    
                    customer.setJ_director_type_document (rs.getString("DOC_TYPE_DIRECTOR"));                    
                    customer.setJ_director_passp_serial (rs.getString("DOC_SERIES_DIRECTOR"));                    
                    customer.setJ_director_passp_number (rs.getString("DOC_NUMBER_DIRECTOR"));                    
                    customer.setJ_director_passp_date_reg (rs.getDate("DOC_DATE_ISSUE_DIRECTOR"));                    
                    customer.setJ_director_passp_place_reg (rs.getString("DOC_ISSUER_DIRECTOR"));
                    customer.setJ_director_passp_date_end (rs.getDate("DOC_DATE_EXPIRE_DIRECTOR"));
                    customer.setJ_director_code_citizenship (rs.getString("CITIZEN_DIRECTOR"));
                    customer.setJ_director_birthday (rs.getDate("BIRTH_DATE_DIRECTOR"));
                    customer.setJ_director_birth_place (rs.getString("BIRTH_PLACE_DIRECTOR"));
                    customer.setJ_director_address (rs.getString("ADDRESS_DIRECTOR"));
                    customer.setJ_chief_accounter_name (rs.getString("NAME_ACCOUNTANT"));
                    customer.setJ_accountant_type_document (rs.getString("DOC_TYPE_ACCOUNTANT"));
                    customer.setJ_accountant_passp_serial (rs.getString("DOC_SERIES_ACCOUNTANT"));
                    customer.setJ_accountant_passp_number (rs.getString("DOC_NUMBER_ACCOUNTANT"));
                    customer.setJ_accountant_passp_date_reg (rs.getDate("DOC_DATE_ISSUE_ACCOUNTANT"));
                    customer.setJ_accountant_passp_place_reg (rs.getString("DOC_ISSUER_ACCOUNTANT"));
                    customer.setJ_accountant_passp_date_end (rs.getDate("DOC_DATE_EXPIRE_ACCOUNTANT"));
                    customer.setJ_accountant_code_citizenship (rs.getString("CITIZEN_ACCOUNTANT"));
                    customer.setJ_accountant_birthday (rs.getDate("BIRTH_DATE_ACCOUNTANT"));
                    customer.setJ_accountant_birth_place (rs.getString("BIRTH_PLACE_ACCOUNTANT"));
                    customer.setJ_accountant_address (rs.getString("ADDRESS_ACCOUNTANT"));
                    customer.setJ_file_name (p_id);                    

                } else {
                	customer.setSign_100("no data found in SSI_LEGAL_ENTITY");
                }
            }
        }
        catch (Exception e) {
            ISLogger.getLogger().error((Object)CheckNull.getPstr(e));
            customer.setSign_100(CheckNull.getPstr(e));
            e.printStackTrace();	            
        }
        finally {
            DbUtils.closeResultSet(rs);
	...
	public static String getTypeDocument(String val) {
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		String doc_Name = "";
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("select t.name_cert d_name from S_CERTIFICATE t where t.code_cert = ? and t.act = 'A'");
			ps.setString(1, val);
			rs = ps.executeQuery();
			if(rs.next()){
				doc_Name = rs.getString("D_NAME");
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static String getNationCode(String id_nation,String neededInfo){
		...
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("select s.nationality,s.map from ss_national_gcp s where s.Map =?");
	...
	public static String getSequence_NextVal(String p_sequence_name) {
		String result = null;
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			...
			ps = c.prepareStatement("SELECT "+p_sequence_name+".nextval id FROM dual");
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public static String getConst1001(String alias) {
		String result = null;
		Connection c = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		try {
			...
			ps = c.prepareStatement("select value url from ss_const where id = 1001 and rownum = 1");
			rs = ps.executeQuery();
			if (rs.next()) {
	...
	public static void setV_const_1001(String v_const_1001) {
		ClientAService.v_const_1001 = v_const_1001;
	}

	public static String getV_const_1001(String alias) {
		if (v_const_1001==null || v_const_1001.equals("")) {
			v_const_1001=getConst1001(alias);
			return v_const_1001;
		} else {
			return v_const_1001;	
		}	
