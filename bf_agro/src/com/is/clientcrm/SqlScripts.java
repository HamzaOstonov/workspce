...
//	CLIENT_TYPES("select id data, name_ru label from SS_CLIENT_TYPES"),
	...
	REGION_DISTRICTS(	"select region_id, distr data, distr_name label from s_distr where act <> 'Z' order by region_id, distr"),
	...
	OPF(				"select opf_id data, name_ru label from S_SPR_OPF1 where act <> 'Z' order by data"),
	...
	//SUBSIDIARY(			"select code data, name label from SS_SUBSIDIARY where branch=info.getbranch and STATE='A' order by code"),
	
	BRANCH_SUBSIDIARIES(		"select branch, code data, name label from SS_SUBSIDIARY where STATE='A' order by branch, code"),
	
	//SUBBRANCH(			"select bank_id data, bank_name label from s_mfo where act <> 'Z' and bank_type=info.getbanktype and tcc_id=info.getbranch and bank_statu in ('10') order by bank_id"),

	BRANCH_SUBBRANCHS(			"select tcc_id branch, bank_id data, bank_name label from s_mfo where act <> 'Z' and bank_type=(select s2.bank_type from s_mfo s2 where s2.bank_id=(select branch from sets where rownum=1)) and bank_statu in ('10') order by tcc_id,bank_id"),
