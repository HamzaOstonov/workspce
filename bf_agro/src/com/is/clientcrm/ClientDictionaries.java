...
//	}
    ...
    public static List<RefData> getNonResidentTypes(Connection c, String alias) {
        return c != null
                ? DbUtils.getRefData(c, SqlScripts.NON_RESIDENT_TYPES.getSql())
                : RefDataService.getRefData(SqlScripts.NON_RESIDENT_TYPES.getSql(), alias);
    }
...
//	}
    ...
    public static List<Soato> getRegion_Districts(String schema){
        Connection c = null;
        ...
        try{
            ...
            preparedStatement = c.prepareStatement(SqlScripts.REGION_DISTRICTS.getSql());
            /* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
             * */
            rs = preparedStatement.executeQuery();
            while (rs.next()) { 
                list.add(new Soato(rs.getString("region_id"), rs.getString("data"), rs.getString("label"), "", "", ""));
            }
        }
        catch (Exception e){
    ...
    public static List<RefData> getSwift(Connection c, String alias) throws SQLException {
    	return c != null
                ? DbUtils.getRefData(c, SqlScripts.SWIFT.getSql())
                : RefDataService.getRefData(SqlScripts.SWIFT.getSql(), alias);
    }

    /*public static List<RefData> getSubsidiary(Connection c, String alias) throws SQLException {
    	return c != null
                ? DbUtils.getRefData(c, SqlScripts.SUBSIDIARY.getSql())
                : RefDataService.getRefData(SqlScripts.SUBSIDIARY.getSql(), alias);
    }
    */
    /*
    public static List<RefData> getSubbranch(Connection c, String alias) throws SQLException {
    	return c != null
                ? DbUtils.getRefData(c, SqlScripts.SUBBRANCH.getSql())
                : RefDataService.getRefData(SqlScripts.SUBBRANCH.getSql(), alias);
    }*/
    ...
    public static Map<Integer, String> getAvailableActions(ClientA client, String username, String pwd, String alias) {
        ...
        try {//"select 2 as deal_group, tc.deal_id,tc.action_id,aa.name from trans_account tc, action_account aa where tc.deal_id = 2 and state_begin = ? and aa.deal_id = 2 and aa.id = tc.action_id"
            ...
            ps.setString(1, client.getSign_registr());
            ps.setString(2, client.getState());
            //ps.setString(3, client.getSign_registr());
...
//		list.add(new RefData("54","ИНН ИП)"));
    ...
    public static List<Soato> getBranch_Subbranchs(String schema){
        Connection c = null;
        PreparedStatement preparedStatement = null;
        ResultSet rs = null;
        List<Soato> list = new ArrayList<Soato>();
        try{
            c = ConnectionPool.getConnection();
            preparedStatement = c.prepareStatement(SqlScripts.BRANCH_SUBBRANCHS.getSql());
            /* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
             * */
            rs = preparedStatement.executeQuery();
            while (rs.next()) { 
                list.add(new Soato(rs.getString("branch"), rs.getString("data"), rs.getString("label"), "", "", ""));
            }
        }
        catch (Exception e){
            logger.error(CheckNull.getPstr(e));
    ...
    public static List<Soato> getBranch_Subsidiaries(String schema){
        Connection c = null;
        PreparedStatement preparedStatement = null;
        ResultSet rs = null;
        List<Soato> list = new ArrayList<Soato>();
        try{
            c = ConnectionPool.getConnection();
            preparedStatement = c.prepareStatement(SqlScripts.BRANCH_SUBSIDIARIES.getSql());
            /* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
             * */
            rs = preparedStatement.executeQuery();
