...
public class AddCstViewCtrl extends GenericForwardComposer {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		add_everywhere$ap_type_document.setModel((new ListModelList(
				com.is.utils.RefDataService.getType_document(alias))));
		...
		add_everywhere$ap_code_tax_org.setModel((new ListModelList(
				com.is.utils.RefDataService.getTax(alias))));
		// add_everywhere$ap_code_bank.setModel((new ListModelList(
		// com.is.utils.RefDataService.getMfo(alias))));
		...
		add_everywhere$ap_code_citizenship.setModel((new ListModelList(
				com.is.utils.RefDataService.getCountry(alias))));
		...
		// ListModelList(com.is.utils.RefDataService.getType_client())));
		add_everywhere$acode_resident.setModel((new ListModelList(
				com.is.utils.RefDataService.getRezCl(alias))));
		addwnd$sproduct.setModel((new ListModelList(com.is.utils.RefDataService
					...
					bt_acc_act.addEventListener(Events.ON_CLICK,
							new EventListener() {
								...
								public void onEvent(Event event)
									...
									com.is.account.PagingListModel acc_model = new com.is.account.PagingListModel(
				...
				bt_block_card_acc.addEventListener(Events.ON_CLICK,
						new EventListener() {
							...
							public void onEvent(Event event) throws Exception {
								...
								try {
									connectionInfo.setBANK_C(ConnectionPool
											.getValue("BANK_C", alias));
									connectionInfo.setGROUPC(ConnectionPool
											.getValue("GROUPC", alias));

									dac.setCARD(((AccInfo) event.getTarget()
											.getAttribute("acc")).getCard());
									dac.setDORMANT_MODE("0");
									dac.setBANK_C(ConnectionPool.getValue(
											"BANK_C", alias));
									dac.setGROUPC(ConnectionPool.getValue(
											"GROUPC", alias));
						...
						new EventListener() {
							...
							public void onEvent(Event event) throws Exception {
								...
								try {
									...
									parameters.setBANK_C(ConnectionPool
											.getValue("BANK_C", alias));
									parameters.setGROUPC(ConnectionPool
											.getValue("GROUPC", alias));
							...
							new EventListener() {
								...
								public void onEvent(Event event)
									...
									Customer cst = CustomerService.getCustomerById(client_id,
											ConnectionPool.getValue("HO", alias), CustomerService.get_alias_ho(alias));
									...
									if (addwnd$sproduct.getValue().compareTo("004") == 0) {
										...
										contract_nmb = null;
										RT = open_card("024", RT, false, new_card_acc);
										if (RT == null)
											return;
										if (cst.getCode_resident().equals("1"))
										open_card("009", RT, true, new_card_acc);
										...
										contract_nmb = null;
										RT = open_card("025", RT, false, new_card_acc);
										if (RT == null)
											return;
										if (cst.getCode_resident().equals("1"))
										open_card("009", RT, true, new_card_acc);
										...
										contract_nmb = null;
										RT = open_card("200", RT, false, new_card_acc);
										if (RT == null)
											return;
				...
				if (fl_show) {
					...
					if (pCustomer.getState() != 2) {
						...
						lc1.setAttribute("alias",
								ConnectionPool.getValue("HO_ALIAS", branch));
						lc2.setAttribute("branch", pCustomer.getBranch());
						lc2.setAttribute("client_id", pCustomer.getId_client());
	...
	public void onClick$tbtn_search() {
		...
		filter.setBranch(ConnectionPool.getValue("HO", alias));
	...
	public void onClick$btn_addacc$accwnd() {
		...
		accwnd$scurracc.setModel((new ListModelList(com.is.utils.RefDataService
	...
	public void onSelect$ap_code_birth_region$addCust(Event evt) {
		...
		addCust$ap_code_birth_distr.setModel((new ListModelList(
				com.is.utils.RefDataService.getDistr(
	...
	private Tclient getTFromBank(Customer cust, Tclient tcust) {
		...
		tcust.setBank_c(ConnectionPool.getValue("BANK_C", alias));
		...
		if (!add_ti && add_ho && !add_br) {
			if (is_br) {
				...
				Customer lg_c = CustomerService.getCustomerById(
						addCustomer$add_cst_id.getValue(),
						ConnectionPool.getValue("HO", alias), halias);
				if (lg_c.getName() != null) {
	...
	public void onClick$add_btn$add_everywhere() throws SAPException,
		...
		//	err += "\nНомер телефона. 998*********";
		//}
		if ((!((add_everywhere$ap_phone_mobile.getValue()).matches("[0-9]*")))
				|| (add_everywhere$ap_phone_mobile.getValue().length() <= 10)) {
			fl_err = true;
			err += "\nНомер телефона должен быть только цифры и длина болше 10 цифр.";
		}
		...
		if (!fl_edit) { // новый клиент
			...
			if (add_ho) {
				...
				Res res = CustomerService.doAction(session.getAttribute("un").toString(),
							...
							new_customer, 1, 0, halias, branch.compareTo(ConnectionPool.getValue(
												"HO", alias)) == 0);
				...
				if (res.getCode() != 0) {
					...
					if (header.getId_client_sap() == null)
					{
						...
						res = CustomerService.doAction_utv(session.getAttribute("un").toString(),
								session.getAttribute("pwd").toString(),
								new_customer, 6, halias, branch.compareTo(ConnectionPool.getValue(
													"HO", alias)) == 0);
					    if (res.getCode() != 0) {
					...
					res = CustomerService.doAction_utv(session.getAttribute("un").toString(),
							session.getAttribute("pwd").toString(),
							new_customer, 2, halias, branch.compareTo(ConnectionPool.getValue(
												"HO", alias)) == 0);
				    if (res.getCode() != 0) {
			...
			if (add_br) {
				if (branch.compareTo(ConnectionPool.getValue("HO", alias)) != 0) {  // branch!=GolovnayaSxema
				...
				if ((branch.compareTo(ConnectionPool.getValue("HO", alias)) == 0)
						&& add_br && add_ho) {
					Branch_id = HO_id;
				}
			...
			if (add_ti) {
				...
				try {
					connectionInfo.setBANK_C(ConnectionPool.getValue("BANK_C",
							alias));
					connectionInfo.setGROUPC(ConnectionPool.getValue("GROUPC",
							alias));
		...
		{ // корректировать сушествуюшего клиента
				...
				Res res = CustomerService
						.doAction(session.getAttribute("un").toString(),
								...
								new_customer, 19, 0, halias, branch
										.compareTo(ConnectionPool.getValue(
												"HO", alias)) == 0);
				if (res.getCode() != 0) {
					alert(res.getName());
					return;
			...
			if (edit_ti) {
				...
				RowType_EditCustomer_Request customerRequest = new RowType_EditCustomer_Request(
						rtc.getCLIENT(), ConnectionPool.getValue("BANK_C",
								alias), rtc.getCLIENT_B(), rtc.getCL_TYPE(),
				...
				try {
					...
					if (customerComplete != null) {
						...
						if (nationMapList == null)
							nationMapList = com.is.utils.RefDataService
	...
	private static void sendUrlRequest(String urlString) throws Exception {
		...
		try {
		      ...
		      LtLogger.getLogger().error("req 3");
		      ...
		      e.printStackTrace();
		      LtLogger.getLogger().error(e.getMessage(), e);
		      //return result;
		    } finally {
		    }
	...
	public void onClick$btn_add$addwnd() {
		...
		if (tcs.getAccount_ccy().equals("000")) {	
			open22618_acc1(
					...
					(branch.compareTo(ConnectionPool.getValue("HO", alias)) == 0) ? tcs
	...
	private String open_card(String card_prod_id, String RT, boolean sec,
		...
		try {
			...
			avalue.setBANK_CODE(ConnectionPool.getValue("BANK_C", alias));
			...
			/*if (card_prod_id == "860" || card_prod_id == "200") {
			...
			//2024.06.10
			base_info.setCCY(tcs.getCard_ccy());
	...
	public void onClick$tbtn_add_card() {
		...
		acfilter.setAcc_bal(cur_branch_customer.getCode_subject().equals("J") ? "22619"
				: "22618");
		acfilter.setBranch(ConnectionPool.getValue("HO", alias));
		acfilter.setCurrency("840");
		String halias = CustomerService.get_alias_ho(alias);
		com.is.account.PagingListModel acc_model = new com.is.account.PagingListModel(
	...
	public void onSelect$ap_code_birth_region$add_everywhere() {
		add_everywhere$ap_code_birth_distr
				.setModel((new ListModelList(com.is.utils.RefDataService
						.getDistr(
								add_everywhere$ap_code_birth_region.getValue(),
								alias))));
	}
	...
	public void onSelect$sproduct$addwnd() {
		...
		List<Account> aModel = AccountService.get_card_accounts_new_card(
				cl_id, branch, addwnd$sproduct.getValue(),
				tietocl.getClient(), tcs.getAccount_ccy());
		addwnd$accGrid.setModel(new ListModelList(aModel));


		/*
		 * String halias = CustomerService.get_alias_ho(alias);
		 * com.is.account.PagingListModel acc_model = new
		 * com.is.account.PagingListModel(0, 100, acfilter, halias);
		 */
		/*
		 * if (aModel.size() != 0) { if
		 * ((addwnd$sproduct.getValue().compareTo("009"
	...
	private void fill_cl_report(Tclient tietocl, String rep_name,
		...
		params.put(
				"HO_BRANCH_NAME",
				com.is.utils.RefDataService
						.getMfo_name(ConnectionPool.getValue("HO", alias),
								halias).get(0).getLabel());
		params.put(
				"HO_BRANCH_ADDRESS",
				com.is.utils.RefDataService
						.getMfo_addr(ConnectionPool.getValue("HO", alias),
								halias).get(0).getLabel());
		...
		params.put("HO_BRANCH", ConnectionPool.getValue("HO", alias));

		/******************* BRANCH DATA *******************/
		params.put("BRANCH_NAME",
				com.is.utils.RefDataService.getMfo_name(branch, halias).get(0)
						.getLabel());
		params.put("BRANCH_ADDRESS",
				com.is.utils.RefDataService.getMfo_addr(branch, halias).get(0)
						.getLabel());
