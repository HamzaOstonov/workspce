...
public class ComJpayment {
	...
	public ComJpayment(int id, int services_list_id, String p_name,
			...
			String budget_account, int district_id) {
		super();
		this.id = id;
	...
	public Payment get_payment()
	{
		...
		pmt.setId(id);
		//pmt.tr_id = tr_id;
		//pmt.setBranch(branch_id);
		//pmt.terminal_id = terminal_id;
		//pmt.provider_id = provider_id;
		pmt.setService_id(services_list_id);
		pmt.setP_name(p_name);
		pmt.setP_number(p_number);
		pmt.setFrom_date(from_date);
		pmt.setTo_date(to_date);
		pmt.setFrom_value(from_value);
		pmt.setTo_value(to_value);
		pmt.setAmount(amount);
		pmt.setCurrency(currency);
		pmt.setDistrict(Integer.toString(district_id));
		//pmt.time_stamp = time_stamp;
		pmt.setState(state);
		return pmt;
	}
