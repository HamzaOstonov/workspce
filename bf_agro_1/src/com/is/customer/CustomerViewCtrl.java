...
public class CustomerViewCtrl extends GenericForwardComposer
{
	...
	private Textbox fid, fbranch, fid_client, fname, fcode_country, fcode_type, fcode_resident, fcode_subject, fsign_registr, fcode_form,  fstate, fp_post_address, fp_passport_serial, fp_passport_number,
	...
	private Datebox fp_birthday, fdate_open, fdate_close;
	private int _startPageNumber = 0;
	private int _totalSize = 0;
	private boolean _needsTotalSizeUpdate = true;
	
	// public TietoCustomer current;
 ...
 */
	...
	public void doAfterCompose(Component comp) throws Exception
	{
		...
		dataGrid.setItemRenderer(new ListitemRenderer()
		{
			...
			public void render(Listitem row, Object data) throws Exception
			{
				 ...
				 * row.appendChild(new
				 * Listcell(pCustomer.getSign_registr()+""));
				...
				/*
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_type()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_serial()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_number()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_place_registration()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_date_registration()+""));
				 * row.appendChild(new Listcell(pCustomer.getP_code_tax_org()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_number_tax_registration()));
				 * row.appendChild(new Listcell(pCustomer.getP_code_bank()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_class_credit()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_citizenship()));
				 * row.appendChild(new Listcell(pCustomer.getP_birth_place()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_capacity()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_capacity_status_date()+""));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_capacity_status_place()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_num_certif_capacity()));
				 * row.appendChild(new Listcell(pCustomer.getP_phone_home()));
				 * row.appendChild(new Listcell(pCustomer.getP_phone_mobile()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_email_address()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_pension_sertif_serial()));
				 * row.appendChild(new Listcell(pCustomer.getP_code_gender()));
				 * row.appendChild(new Listcell(pCustomer.getP_code_nation()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_birth_region()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_birth_distr()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_type_document()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_passport_date_expiration()+""));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_adr_region()));
				 * row.appendChild(new
				 * Listcell(pCustomer.getP_code_adr_distr()));
		...
		p_code_bank.setModel((new ListModelList(com.is.utils.RefDataService.get_cur_bank_Mfo(alias))));
	...
	private void refreshModel(int activePage)
	{
		...
		if (model.getSize() > 0)
		{
			...
			// this.current = (TietoCustomer) model.getElementAt(0);
			sendSelEvt();
		}
	}
	...
	public void onClick$btn_tst()
	{
		...
		// IssuingPortProxy pp = new
		// IssuingPortProxy("http://iss_ws:visaipak@10.11.0.63:8080/issuingws/services/Issuing");
		IssuingPortProxy pp = new IssuingPortProxy(ConnectionPool.getValue("TIETO_HOST", alias),
				ConnectionPool.getValue("EMPC_TIETO_HOST_USERNAME", alias),
				ConnectionPool.getValue("EMPC_TIETO_HOST_PASSWORD", alias));
		// IssuingPortProxy pp = new
		// IssuingPortProxy("http://iss_ws:visaipak@10.11.0.63:8083/issuingws/services/Issuing");
		// IssuingPortProxy pp = new
		// IssuingPortProxy("http://127.0.0.1:8083/issuingws/services/Issuing");
		try
		{
	...
	{
		com.is.LtLogger.getLogger().info(ap_passport_number.getValue());
