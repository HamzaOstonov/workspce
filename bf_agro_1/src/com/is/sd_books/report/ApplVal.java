...
public class ApplVal extends PoiReport {
	...
	public AMedia getRepmd(Map<String, Object> params, Connection c,
		...
		try {
			...
			//System.out.println("-----Appval2:"+Integer.valueOf((int) Double.parseDouble(params.get("book_id")+"")));
			cs.setString(2, "ApplVal.doc");
			cs.setString(3, "01");
			...
			//System.out.println("---AppVal525:"+params.get("book_id"));
			...
			if (rs.next()) {
				...
				String name_dep1=name_dep.substring(0,ind2);
				...
				Date dog_dat = rs.getDate("dog_dat");
			     ...
			     Date date = rs.getDate("date_open");
				
				replaceText("<DATE_OPEN>",(date != null) ? df.format(date) : emptyString, range);
		...
		try {
			c = ConnectionPool.getConnection(params.get("un").toString(),
					params.get("pwd").toString(), params.get("alias")
							.toString());
		//	System.out.println("------MAP--3:"+params.get("un").toString());
