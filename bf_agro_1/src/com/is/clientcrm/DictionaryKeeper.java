...
public class DictionaryKeeper {
	...
	public static HashMap<String, String> getClientCodeLetters() {
		if (clientCodeLetters == null || clientCodeLetters.isEmpty()) {
			HashMap<String,String> list = new HashMap<String,String>();
		
			 for (Map.Entry<String, String> set :
				 clientLetters.entrySet()) {
	            // Printing all elements of a Map
	            //System.out.println(set.getKey() + " = " + set.getValue());
	            list.put(set.getValue(), set.getKey());
	        }
			clientCodeLetters=list;
		}
		return clientCodeLetters;
	}
	...
	private static HashMap<String, String> clientCodeLetters;
	...
	//private static List<RefData> subsidiary;
	private static HashMap<String, List<RefData>> branch_subsidiaries;
	
	//private static List<RefData> subbranch;
	private static HashMap<String, List<RefData>> branch_subbranchs;
	
	private static HashMap<String, List<RefData>> region_districts;
	...
	public void initLists() {
		...
		try {
			...
			if (region_districts == null) {
				HashMap<String, List<RefData>> myMap = new HashMap<String, List<RefData>>();
				String br="_";
				/* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
	             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
	             * */
				List<Soato> myList = ClientDictionaries.getRegion_Districts(alias);
				List<RefData> subList=null;
				for (Soato st : myList) {
                    if (!br.equals(st.getKod_soat()))					
					{
                    	if (!br.equals("_"))
                    	  myMap.put(br, subList);                   	
                    	subList = new ArrayList<RefData>();
					}                    
                    subList.add(new RefData(st.getKod_gni(), st.getRegion()));                    
					br=st.getKod_soat();
				}
            	if (!br.equals("_"))
              	  myMap.put(br, subList);
				
            	region_districts = myMap;
			}
			...
			//	subsidiary = ClientDictionaries.getSubsidiary(tm.getConnection(), alias);
			if (branch_subsidiaries == null) {
				HashMap<String, List<RefData>> myMap = new HashMap<String, List<RefData>>();
				String br="_";
				/* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
	             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
	             * */
				List<Soato> myList = ClientDictionaries.getBranch_Subsidiaries(alias);
				List<RefData> subList=null;
				for (Soato st : myList) {
                    if (!br.equals(st.getKod_soat()))					
					{
            	...
            	if (!br.equals("_"))
              	  myMap.put(br, subList);
            	branch_subsidiaries = myMap;
			}
			
			//if (subbranch == null || subbranch.isEmpty())
			//	subbranch = ClientDictionaries.getSubbranch(tm.getConnection(), alias);

			if (branch_subbranchs == null) {
				HashMap<String, List<RefData>> myMap = new HashMap<String, List<RefData>>();
				String br="_";
				/* shu Soato klassdan foydalanamiz. boshqa klass yasamaslik uchun.
	             * BRANCH=kod_soat, DATA=kod_gni, LABEL=region
	             * */
				List<Soato> myList = ClientDictionaries.getBranch_Subbranchs(alias);
				List<RefData> subList=null;
				for (Soato st : myList) {
                    if (!br.equals(st.getKod_soat()))					
					{
	...
	public static List<RefData> getDistrict_by_region(String region) {
		HashMap<String, List<RefData>> r_d = getRegion_districts();
		List<RefData> subList = new ArrayList<RefData>();
		if (r_d.containsKey(region))
			subList = r_d.get(region);
		return subList;
	}

	public static List<RefData> getSubsidiary_by_branch(String branch) {
		HashMap<String, List<RefData>> r_d = getBranch_subsidiaries();
		List<RefData> subList = new ArrayList<RefData>();
		if (r_d.containsKey(branch))
			subList = r_d.get(branch);
		return subList;
	}

	
	public static List<RefData> getSubbranch_by_branch(String branch) {
		HashMap<String, List<RefData>> r_d = getBranch_subbranchs();
		List<RefData> subList = new ArrayList<RefData>();
		if (r_d.containsKey(branch))
			subList = r_d.get(branch);
		return subList;
	...
	/*public static void setSubsidiary(List<RefData> subsidiary) {
		DictionaryKeeper.subsidiary = subsidiary;
	}
    */
	/*public static List<RefData> getSubsidiary() {
		return subsidiary;
	}*/

	//public static void setSubbranch(List<RefData> subbranch) {
	//	DictionaryKeeper.subbranch = subbranch;
	//}

	//public static List<RefData> getSubbranch() {
    //	return subbranch;
	//}


	public static void setBranch_subbranchs(HashMap<String, List<RefData>> branch_subbranchs) {
		DictionaryKeeper.branch_subbranchs = branch_subbranchs;
	}

	public static HashMap<String, List<RefData>> getBranch_subbranchs() {
		return branch_subbranchs;
	}

	public static void setBranch_subsidiaries(HashMap<String, List<RefData>> branch_subsidiaries) {
		DictionaryKeeper.branch_subsidiaries = branch_subsidiaries;
	}

	public static HashMap<String, List<RefData>> getBranch_subsidiaries() {
		return branch_subsidiaries;
	}

	public static void setRegion_districts(HashMap<String, List<RefData>> region_districts) {
		DictionaryKeeper.region_districts = region_districts;
	}

	public static HashMap<String, List<RefData>> getRegion_districts() {
