...
import com.is.clientcrm.addinfo.Parameter;
...
public class ParameterListService {
	...
	private static String msql ="SELECT * FROM CLIENT_ADDINFO_LIST_TEMPLATE ";
	...
	public static List<ParameterList> getParameterlists(Parameter p)  {
		...
		try {
			...
			ps = c.prepareStatement(
					"SELECT * FROM (" +
						"SELECT tmpl.*, lis.branch, lis.client_id, lis.param_value, lis.state " +
						"FROM " +
			...
			while (rs.next()) {
				...
				if (pl == null || pl.getList_id() != listid) {
					if (pl != null) {
						list.add(pl);
					}
	...
	*/
	public static List<ParameterListTemplate> getParameterListTemplates(Parameter p)  {
		...
		try {
			c = ConnectionPool.getConnection();
			ps = c.prepareStatement("SELECT * FROM CLIENT_ADDINFO_LIST_TEMPLATE t where t.client_type = ? and t.param_id = ? order by t.param_ord");
			ps.setString(1, p.getClient_type());
	...
	public static List<ParameterListTemplate> getParameterListTemplate()  {
		...
		try {
			c = ConnectionPool.getConnection();
			Statement s = c.createStatement();
			ResultSet rs = s.executeQuery("SELECT * FROM CLIENT_ADDINFO_LIST_TEMPLATE");
			while (rs.next()) {
				list.add(new ParameterListTemplate(
						rs.getString("client_type"),
						rs.getString("param_id"),
	...
	public static Res doAction(String un,String pw, ParameterListTemplate parameterlisttemplate,int actionid) {
         ...
         try {
			...
			if(!CheckNull.isEmpty(parameterlisttemplate.getParam_visible())){
				cs.setString(1, "PARAM_VISIBLE");  cs.setString(2,parameterlisttemplate.getParam_visible()); cs.execute();
			}
	...
	public static ParameterListTemplate getParameterListTemplate(int parameterlisttemplateId) {
		...
		try {
			c = ConnectionPool.getConnection();
			PreparedStatement ps = c.prepareStatement("SELECT * FROM CLIENT_ADDINFO_LIST_TEMPLATE WHERE id=?");
			...
			if (rs.next()) {
				...
				parameterlisttemplate.setParam_visible(rs.getString("param_visible"));
	...
	public static ParameterListTemplate create(ParameterListTemplate parameterlisttemplate)  {
		...
		try {
			...
			ps.setString(13, parameterlisttemplate.getParam_visible());
	...
	public static void update(ParameterListTemplate parameterlisttemplate)  {
		...
		try {
			c = ConnectionPool.getConnection();
			PreparedStatement ps = c.prepareStatement("UPDATE CLIENT_ADDINFO_LIST_TEMPLATE SET client_type = ?, param_id = ?, param_list_teg = ?, param_select = ?, param_type = ?, param_mask = ?, param_def_value = ?, param_ord = ?, param_mandatory = ?, param_align = ?, param_constraints = ?, param_enable = ?, param_visible = ?, param_visible_t = ?, param_actions = ?, param_act_runatstart = ?  WHERE id=?");
			ps.setString(1, parameterlisttemplate.getClient_type());
			ps.setString(2, parameterlisttemplate.getParam_id());
			ps.setString(3, parameterlisttemplate.getParam_list_teg());
			ps.setString(4, parameterlisttemplate.getParam_select());
	...
	public static void remove(ParameterListTemplate parameterlisttemplate)  {
		...
		try {
			c = ConnectionPool.getConnection();
			PreparedStatement ps = c.prepareStatement("DELETE FROM CLIENT_ADDINFO_LIST_TEMPLATE WHERE id=?");
