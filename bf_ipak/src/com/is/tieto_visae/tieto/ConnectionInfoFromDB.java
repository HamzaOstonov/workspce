...
public class ConnectionInfoFromDB implements ConnectionInfoHandler {
	...
	public WSConnectionInfo getWSConnectionInfo() {
		...
		try {
			...
			ps = c.prepareStatement("select name, value from bf_visa_connection_sets where name in (?, ?, ?, ?, ?)");
		...
		connectionInfo = new WSConnectionInfo();
		connectionInfo.setHost(ConnectionPool.getValue("TIETO_VISA_HOST"));
		connectionInfo.setLogin(ConnectionPool.getValue("TIETO_VISA_HOST_USERNAME"));
		connectionInfo.setPassword(ConnectionPool.getValue("TIETO_VISA_HOST_PASSWORD"));
		connectionInfo.setBankC(ConnectionPool.getValue("VISA_BANK_C"));
		connectionInfo.setGroupC(ConnectionPool.getValue("VISA_GROUPC"));
		
		return connectionInfo;
	}

	public ConnectionInfoFromDB() {
		super();
