...
public class CustomerService
{
	...
	public static List<RefData> getProduct(String branch)
	{
		return RefDataService.getRefData("select code data, name label from BF_globuz_CARD_SETTING where flag_user_view='1' and FLAG_CLIENT_TYPE='P' order by code", branch);
	}
	...
	public static Res doAction(String un, String pw, Customer customer, int actionid, int utv_actionid, String alias, Boolean selfBranch)
	{
		...
		catch (Exception e)
		{
			...
			{
				ISLogger.getLogger().error(CheckNull.getPstr(g));
	...
	public static Customer getCustomerById(String customerId, String branch, String alias)
	{
		...
		{
			c = ConnectionPool.getConnection(alias);
			PreparedStatement ps = c.prepareStatement("SELECT * FROM v_bfcustomer WHERE id_client=? and branch = ?");// and
																										// branch=?");
			ps.setString(1, customerId);
			ps.setString(2, branch);
			ResultSet rs = ps.executeQuery();
	...
	public static Res update_lnk_set_acc(String un, String pwd, String branch, String br_id, GlobuzAccount acc, String alias, globus.IssuingWS.IssuingPortProxy issuingPortProxy)
	{
		Res res  = new Res();
		Connection c = null;
		PreparedStatement ps = null;
...
//			ps.setString(5, last);
                                                                 ...
                                                                 String branch, String card_code, String t_client_id)
	{
		...
		{
			ISLogger.getLogger().error(CheckNull.getPstr(exp));
