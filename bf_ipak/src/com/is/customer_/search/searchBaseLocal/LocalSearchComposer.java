		...
		if (main.isVisible()) {
			if (interactor.getCustomer().getSign_date_open() == 2) {
				if (interactor.getCustomer().getDate_open() == null) {
					Messagebox.show("¬ведите начальна€ дата утверждени€");
					return;
				}
				if (interactor.getCustomer().getDate_open1() == null) {
					Messagebox.show("¬ведите конечьна€ дата утверждени€");
					return;
				}
			}
			if (interactor.getCustomer().getSign_date_close() == 2) {
				if (interactor.getCustomer().getDate_close() == null) {
					Messagebox.show("¬ведите начальна€ дата закрыти€");
					return;
				}
				if (interactor.getCustomer().getDate_close1() == null) {
					Messagebox.show("¬ведите конечьна€ дата закрыти€");
					return;
				}
			}
			...
			refreshModel(0);
			if (model.getSize() == 0)
				Messagebox.show("ƒеловой партнер не найден");
			else
			{
