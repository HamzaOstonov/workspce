...
import com.is.useractionlog.UserActionLog;
...
public class ViewController extends GenericForwardComposer {
    private Map<Integer,String> sessionMap = Collections.synchronizedMap(new HashMap<Integer, String>());
	...
	private Label res, headwnd$lblusr, headwnd$lbldate, headwnd$lblversion;
	...
	public void login() {
		...
		if (win$tb_username.isValid() && win$tb_password.isValid()) {
			...
			if (res.getCode() == 2)
				alert("Срок действия Вашего пароля подходит к концу.\n\nПожалуйста смените пароль.");
			
			/*OBJ_Res res = new OBJ_Res();
			User user = new User();
			user.setAlias("BANK1066");
			user.setBranch("01066");
			...
			headwnd$lblusr.setValue(currusr.getFull_name() + " " + currusr.getTitle());
			headwnd$lblversion.setValue("Версия:" + ConnectionPool.getValue("MUNIS_WEB_VERSION"));
			headwnd$lbldate.setValue(df.format(new java.util.Date()));
			if (currusr.getId() > 0) {
                ...
                if (sessionMap.get(currusr.getId()) == null){
                    sessionMap.put(currusr.getId(), hr.getRequestedSessionId());
                }
                else
                    return;
				...
				// UserActionLog(currusr.getId(),win$tb_username.getText(),ipAddress,1,
				// Long.valueOf(1), "Удачный вход в систему",
				// currusr.getBranch()));
				// UserService.UsrLog(new
				// UserActionLog(currusr.getBranch(),uid,win$tb_username.getText(),ipAddress,1),currusr.getAlias());
				// UserService.UsrLog(new UserActionsLog(currusr.getId(),
				// win$tb_username.getText(), ipAddress, 1, Long.valueOf(1),
				// "Удачный вход в систему", currusr.getBranch()));
				// UserService.UsrLog(new UserActionsLog(currusr.getBranch(),
				// uid, win$tb_username.getText(), ipAddress, 1),
				// currusr.getAlias());
				// UserService.UsrLog(new
				// UserActionsLog(uid,win$tb_username.getText(),hr.getRemoteAddr(),1));
				...
				String req = "http://" + exec.getLocalAddr() + ":" + exec.getLocalPort() + "/bf/UsersControlServlet?uid="
						+ (Integer) sessionController.getSessionObject("uid") + "&session="
						+ exec.getSession().getNativeSession().hashCode() + "&action=login";
				req = "http://" + "127.0.0.1" + ":" + exec.getLocalPort() + "/bf/UsersControlServlet?uid="
						+ (Integer) sessionController.getSessionObject("uid") + "&session="
						+ exec.getSession().getNativeSession().hashCode() + "&action=login";
				...
				// UserService.UsrLog(new UserActionsLog(currusr.getBranch(),
				// uid, win$tb_username.getText(), hr.getRemoteAddr(), 2),
				// currusr.getBranch());
	...
	private void addModule(int userId, String lang) {
		...
		try {
			...
			// m.setId("btn_help");
			/*
			 * m.addEventListener(Events.ON_CLICK, new ClientPListener() { public
			...
			// ResultSet rs =
			// s.executeQuery("select level,'md'||t.id
			// mid,t.mtype,t.name,t.mname,t.icon from modules t START WITH
			// t.parentid IS NULL CONNECT BY PRIOR t.id = t.parentid ORDER
			// SIBLINGS BY t.id");
			PreparedStatement ps = c.prepareStatement("select level,'md'||t.id mid,t.mtype,t.name_" + lang
					+ ",t.mname,t.icon, t.extparam,t.help_url " + "from (select * from bf_modules m "
			...
			while (rs.next()) {
				if (rs.getString("mtype").equals("0")) {
					...
					mi.addEventListener(Events.ON_CLICK, new EventListener() {
						public void onEvent(Event event) throws Exception {
							...
							if (menuItem.getValue().equals("report.zul")) {
								...
								// System.out.println("Svitched to
								// module:"+menuItem.getLabel());
								module_caption.setValue(menuItem.getLabel());
								ical.setSrc(menuItem.getValue() + "?ht=" + desktopHeight
										+ (menuItem.getAttribute("exppar") != null
												? menuItem.getAttribute("exppar").toString() : ""));
					...
					mt.addEventListener(Events.ON_CLICK, new EventListener() {
						public void onEvent(Event event) throws Exception {
							...
							if (menuItem.getValue().equals("report.zul")) {
								...
								module_caption.setValue(menuItem.getLabel());
								ical.setSrc(menuItem.getValue() + "?ht=" + desktopHeight + "&exppar="
										+ (menuItem.getAttribute("exppar") != null
												? menuItem.getAttribute("exppar").toString() : ""));
							}
							// System.out.println(menuItem.getValue());
					...
					// System.out.println("Width "+menubar.getWidth()+" label
					// "+lmenulength*13+" "+rs.getString("name_ru"));
					// toolbar
				}
				// mmdl.appendChild(mi);
	...
	private boolean lg(String un, String pwd) {
		...
		try {
			...
			// System.out.println("SELECT * FROM v_bf_bank_users WHERE
			// username="+un+" and state=1");
			PreparedStatement ps = c.prepareStatement("SELECT * FROM v_bf_bank_users WHERE username=? and state=1");

			// PreparedStatement ps =
			// c.prepareStatement("SELECT * FROM users WHERE username=? and
			// state=1");
			ps.setString(1, un);
			ResultSet rs = ps.executeQuery();
			if (rs.next()) {
	...
	public void onClick$btn_save$pwdwnd() {
		...
		Res rs = UserService.chPwd(currusr.getUser_name(), pwdwnd$newpwd.getValue(), pwdwnd$newpwd1.getValue(),
				pwdwnd$oldpwd.getValue());
