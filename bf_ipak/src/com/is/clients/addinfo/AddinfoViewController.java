...
import com.is.clients.addinfo.list.ParameterListArray;
import com.is.clients.addinfo.list.ParameterListService;
import com.is.clients.addinfo.list.ParameterListTemplate;
//import com.is.clients.persons.ClientPersonCardViewCtrl;
//import com.is.clients.persons.PersonMap;
//import com.is.clients.persons.PersonMapService;
...
public class AddinfoViewController extends GenericForwardComposer {
	...
	private Toolbarbutton btn_refresh, btn_save_top, btn_save_bottom, btn_approve, btn_noapprove, btn_delete,btn_restore;
	...
	private String branch1,alias,lang, un, pw;
	public User usr;
	...
	*/
	...
	public void render(Listitem row, Object data) throws Exception {
		...
		//alert("params"+pParameterList.getParameters().size());
	...
	private String client_code_subject = null;
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        try {
			ParameterService.synchronize(current, un, pw);
		...
		addinfoObj.addToSource(
		    ...
		    "{",
			    "try {",
			    	...
			    	"com.is.clients.addinfo.Parameter p = (com.is.clients.addinfo.Parameter)comp.getAttribute(\"parameter\");",
		    ...
		    "{",
			    "try {",
			    	...
			    	"com.is.clients.addinfo.list.ParameterListArray p = (com.is.clients.addinfo.list.ParameterListArray)comp.getAttribute(\"parameter\");",
				    "p.setParam_mandatory(bool);",
				    "comp.setAttribute(\"parameter\", p);",
			    "} catch (java.lang.Exception e) { e.printStackTrace(); }",
		    "}",
		    ...
		    "{",
		    	...
		    	"} catch (Exception e) {e.printStackTrace();}",
		    "}",
            "public String getLicensingDistrict(org.zkoss.zk.ui.Component comp)",
            "{",
                "com.is.clients.addinfo.AddinfoViewController controller = (AddinfoViewController) comp.getAttribute(\"clientaddinfomain$composer\");",
                "com.is.clients.addinfo.Clientmap current = controller.getCurrent();",
                "return com.is.clients.addinfo.AddinfoUtils.getLicensingAuthorityDistrict(current.getBranch(),current.getId_client(),current.getAlias());",
            "}"
                /*,

		    /*,
		    "public java.util.List<com.is.clients.addinfo.Parameter> parameters setParameterMandatory ( java.lang.Long paramid, java.lang.Integer m, org.zkoss.zul.Div addinfo, java.util.List<com.is.clients.addinfo.Parameter> parameters)",
		    "{",
    ...
    /*public String getLicensingDistrict(Component comp){
        com.is.clients.addinfo.AddinfoViewController controller = (AddinfoViewController) comp.getAttribute("clientaddinfomain$composer");
        com.is.clients.addinfo.Clientmap current = controller.getCurrent();
        return com.is.clients.addinfo.AddinfoUtils.getLicensingAuthorityDistrict(current.getBranch(),current.getId_client(),current.getAlias());
    }*/
    ...
    private void refreshModel() {
		ISLogger.getLogger().error("Load Parameters " + current);

		Res resl = ParameterService.loadParameters(current.getBranch(),
    			current.getId_client(), client_code_subject, un, pw, current.getAlias());
		if (resl.getCode() != 0) {
			alert(resl.getName());
			return;
		}
    ...
    public void createAddInfo() {
		...
		parameters = ParameterService.getParameters(current.getBranch(), current.getId_client(), 
				current.getCode_subject());
		oldparameters = ParameterService.getParameters(current.getBranch(), current.getId_client(), 
				current.getCode_subject());
		ISLogger.getLogger().error("Parameters group size " + parametergroups.size());
		try {
			for (ParameterGroup parametergroup : parametergroups) {
				List<Parameter> params = ParameterService
                        .getParametersByGroup(parameters, parametergroup);
				ISLogger.getLogger().error("params size " + params.size());
				...
				if (params.size() > 0) {
					...
					for (int i = 0; i < params.size(); i++) {
						if (params.get(i).getParam_visible() == 1) {
						...
						if (params.get(i).getParam_type().equals("DATE")){
							...
							if (p.getParam_act_runatstart() == 1) {
								...
								if (!CheckNull.isEmpty(select)){
									ListModelList list = new ListModelList(ParameterService.getListForCombobox(select, un, pw, current.getAlias()));
									c.setModel(list);
							...
							if (p.getParam_act_runatstart() == 1)
							    addinfoObj.invoke( "run"+t.getId(), t, addinfo, usr, 1);
						} else if (params.get(i).getParam_type().equals("PERSONGRID")){
	...
	public Boolean saveAddInfo(int needcorrect) {
		...
		try {
			...
			Res rs = ParameterService.saveAddInfo(params, current, oldparameters, needcorrect, paramlistparams, un, pw);
    		if (rs.getCode() != 0) {
    			alert(rs.getName());
    			res = false;
    		}  else {
	...
	private Checkbox getCHECKBOX(ParameterGroup parametergroup, Parameter p) {
		...
		/*
		addinfoObj.addToSource(
				"public java.util.List<com.is.clients.addinfo.Parameter> run"+c.getId()+"(org.zkoss.zul.Checkbox comp, org.zkoss.zul.Div addinfo, java.util.List<com.is.clients.addinfo.Parameter> parameters) ",
			    "{",
	...
	private Textbox getNUMBER(ParameterGroup parametergroup, Parameter p) {
		...
		/*
		addinfoObj.addToSource(
				"public java.util.List<com.is.clients.addinfo.Parameter> run"+n.getId()+"(org.zkoss.zul.Textbox comp, org.zkoss.zul.Div addinfo, java.util.List<com.is.clients.addinfo.Parameter> parameters) ",
			    "{",
	...
	private Hbox getCHECKLIST(ParameterGroup parametergroup, Parameter p) {
		...
		b.addEventListener("onClick", new EventListener() {
			public void onEvent(org.zkoss.zk.ui.event.Event event) throws Exception {
				...
				String select = p.getParam_select().toUpperCase();
				select = ParameterService.setParamsInSQL(parameters, select, current);
				checklistwnd$checklist.setModel(new BindingListModelList(ParameterService.getListForCheckList(select, t.getValue(), un, pw, alias), true));
				for (int i = 0; i < checklistwnd$checklist.getModel().getSize(); i++) {
					CheckListRefData chd = (CheckListRefData)checklistwnd$checklist.getModel().getElementAt(i);
					if (chd.getIsChecked()) {
						System.out.println(chd.getIsChecked()+" - "+chd.getData()+" - "+chd.getLabel());
	...
	public Div getLIST(ParameterGroup parametergroup, Parameter p) {
		...
		/*for (int i = 0; i < listparams.size(); i++) {
			ISLogger.getLogger().warn("*** listid="+listparams.get(i).getList_id()+" "+listparams.get(i).getParam_id()+" "+listparams.get(i).getParam_visible()+" "+listparams.get(i).getParameters().size()); 
			for (int j = 0; j < listparams.get(i).getParameters().size(); j++) {
				ISLogger.getLogger().warn("*********** listid="+listparams.get(i).getParameters().get(j).getParam_list_teg()+" "+listparams.get(i).getParameters().get(j).getParam_id()+" "+listparams.get(i).getParameters().get(j).getParam_visible_t()+" "+listparams.get(i).getParameters().get(j).getParam_value()); 
	...
	public void showListWindow(ParameterGroup parametergroup, Parameter p, ParameterList pl, List<ParameterList> listparams) {
		...
		for (int i = 0; i < pl.getParameters().size(); i++) {
			if (pl.getParameters().get(i).getParam_visible() == 1) {
	...
	public String getListToolbar(ParameterListArray p) {
		...
		if (p.getParam_type().equalsIgnoreCase("COMBOBOX")) {
			...
			if (reflist.containsKey(p.getParam_id()+"_"+p.getParam_list_teg())) {
				...
				String select = p.getParam_select().toUpperCase();
				select = ParameterService.setParamsInSQL(parameters, select, current);
				spr = ParameterService.getListForCombobox(select, un, pw, alias);
	...
	public void test() throws InterruptedException {
        //Messagebox.show(addinfo.getPage().getFellow("clientaddinfomain").getId());
	    /*AddinfoViewController controller = new AddinfoViewController();
	    com.is.clients.addinfo.Clientmap clientmap = controller.getCurrent();
        AddinfoUtils.getLicensingAuthorityDistrict(
                current.getBranch(),
                current.getId_client(),
                current.getAlias());*/
    }

    public Clientmap getCurrent(){
        return this.current;
    }
}
