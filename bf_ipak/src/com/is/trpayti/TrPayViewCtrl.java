...
import com.is.utilsti.RefDataService;
...
public class TrPayViewCtrl extends GenericForwardComposer {
				...
				for (int i = 0; i < la.size(); i++) {
					...
					bt.addEventListener(Events.ON_CLICK, new EventListener() {
						...
						public void onEvent(Event event) throws Exception {
							...
							if (TrPayService.get_confirm_action(
									144,
											...
											new EventListener() {
												public void onEvent(Event e) {
													...
													if (Messagebox.ON_YES
															.equals(e.getName())) // if
																					// (Messagebox.ON_YES.equals(e.getName()))
													{
	...
	public void authomat_step(Cur_cont cur_cont) {
		...
		try {
			...
			UserService.UsrLog(new UserActionsLog(uid, (uname == null) ? un : uname , curip, 5, 1, log,
					branch1));
	...
	public void onClick$btn_print_duplicate() {
		...
		Ti_Duplicate duplicate = TclientService.getDuplicate(
	...
	public void onClick$btn_print_duplicate_d() {
		...
		Ti_DuplicateDebit duplicate_debit = TclientService.getDuplicateDebit(
