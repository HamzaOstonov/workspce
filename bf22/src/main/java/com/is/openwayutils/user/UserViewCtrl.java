...
import com.is.openwayutils.utils.RefRenderer;
...
*/
      ...
      // subbranch.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getSusidiary())));
      // abranch.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getMfo())));
       //asubbranch.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getSusidiary())));
       fbranch.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.get_ipak_Mfo(alias))));
       //fsubbranch.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getSusidiary())));
       //fptype.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getDocType())));
       //aptype.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getDocType())));
       //ptype.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getDocType())));
...
public void onDrop$left$rlwnd(DropEvent e){
    if (e.getDragged() instanceof Listitem) {
            ...
            UserService.UsrLog(new UserActionsLog(uid, uname, curip, 3, 1, "Удалена роль "+role.getName()+" пользователя "+current.getUser_name()+" филиала "+current.getBranch() ,branch1), alias);
...
public void onClick$bt_add_role$rlwnd()
{
    ...
    UserService.UsrLog(new UserActionsLog(uid, uname, curip, 3, 1, "Добавлена роль "+cur_role_id.getName()+" пользователю "+current.getUser_name().toUpperCase()+" филиала "+current.getBranch() ,branch1), alias);
...
public void onClick$bt_rem_role$rlwnd()
{
    ...
    UserService.UsrLog(new UserActionsLog(uid, uname, curip, 3, 1, "Удалена роль "+cur_role_id.getName()+" пользователя "+current.getUser_name()+" филиала "+current.getBranch() ,branch1), alias);
