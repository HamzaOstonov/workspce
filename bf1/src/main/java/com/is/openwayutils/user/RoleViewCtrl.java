...
//    	left.setItemRenderer(new com.is.openwayutils.utils.RefListRenderer());
//    	right.setItemRenderer(new com.is.openwayutils.utils.RefListRenderer());
...
//    	right.setModel((new ListModelList(RefDataService.getPrType())));
		...
		caccess.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getAccess(alias))));
		addwnd$caccess.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.getAccess(alias))));
	...
	public void onClick$btn_save(){
		...
		{
			UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Роль ["+old_role_name+"] переименована в ["+current.getName()+"]" ,branch), alias);
		}
		if (old_access != Integer.parseInt( caccess.getSelectedItem().getValue().toString()))
		{
			UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Для роли ["+current.getName()+"] доступ изменен с ["+old_role_access_name+"] на ["+caccess.getSelectedItem().getLabel()+"]", branch), alias);
		}
	...
	public void onClick$btn_del(){
		...
		UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Удалена роль ["+current.getName()+"]" ,branch), alias);
		refresh();
	}
	...
	public void onClick$btn_save$addwnd(){
		...
		UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Добавлена роль ["+current.getName()+"]" ,branch), alias);
    ...
    public void onDrop$right(DropEvent e){
        if (e.getDragged() instanceof Listitem) {
               ...
               // System.out.println("Left add role "+current.getId()+ " module "+module.getId());
                ...
                UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Роли ["+current.getName()+"] добавлен модуль ["+module.getName()+"]", branch), alias);
    ...
    public void onDrop$left(DropEvent e){
        if (e.getDragged() instanceof Listitem) {
               ...
               // System.out.println("right remove role "+current.getId()+ " module "+module.getId());
                ...
                UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Из роли ["+current.getName()+"] удален модуль ["+module.getName()+"]", branch), alias);
    ...
    public void onDoubleClick$right() {
    	...
    	actwnd$deal_id.setModel((new ListModelList(com.is.openwayutils.utils.RefDataService.get_deal_id(alias, 144))));
    ...
    public void onDrop$right$actwnd(DropEvent e){
        if (e.getDragged() instanceof Listitem) {
               ...
               // System.out.println("Left add role "+current.getId()+ " module "+module.getId());
                ...
                UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Для роли ["+current.getName()+"], модуля ["+currmodule.getName()+"] добавлено действие ["+action.getName()+"] подгруппы ["+deal_id_desc+"]", branch), alias);
    ...
    public void onDrop$left$actwnd(DropEvent e){
        if (e.getDragged() instanceof Listitem) {
               ...
               // System.out.println("right remove role "+current.getId()+ " module "+module.getId());
                ...
                UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Из роли ["+current.getName()+"], модуля ["+currmodule.getName()+"] удалено действие ["+action.getName()+"] подгруппы ["+deal_id_desc+"]", branch), alias);
            	actwnd$right.setModel((new ListModelList(UserService.getActionInRole(current.getId(),currmodule.getId(),alias, deal_id))));
            	actwnd$left.setModel((new ListModelList(UserService.getActionNotInRole(current.getId(),currmodule.getId(),alias, deal_id))));
        }
    }	
    ...
    public void onClick$bt_add_mod()
    {
        ...
        UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Роли ["+current.getName()+"] добавлен модуль ["+cur_mod.getName()+"]", branch), alias);
    ...
    public void onClick$bt_rem_mod()
    {
        ...
        UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Из роли ["+current.getName()+"] удален модуль ["+cur_mod.getName()+"]", branch), alias);
    ...
    public void onClick$bt_add_act$actwnd()
    {
        ...
        UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Для роли ["+current.getName()+"], модуля ["+currmodule.getName()+"] добавлено действие ["+cur_act.getName()+"] подгруппы ["+deal_id_desc+"]", branch), alias);
    ...
    public void onClick$bt_rem_act$actwnd()
    {
        ...
        UserService.UsrLog(new UserActionsLog(uid, uname, curip, 4, 1, "Из роли ["+current.getName()+"], модуля ["+currmodule.getName()+"] удалено действие ["+cur_act.getName()+"] подгруппы ["+deal_id_desc+"]", branch), alias);
    	actwnd$right.setModel((new ListModelList(UserService.getActionInRole(current.getId(),currmodule.getId(),alias, deal_id))));
    	actwnd$left.setModel((new ListModelList(UserService.getActionNotInRole(current.getId(),currmodule.getId(),alias, deal_id))));
    }
