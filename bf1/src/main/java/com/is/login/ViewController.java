...
public class ViewController extends GenericForwardComposer
{
	...
	public void doAfterCompose(Component comp) throws Exception
	{
		 ...
		 * System.out.println("���� "+Executions.getCurrent().getUserAgent());
		 * }else{
		 * System.out.println("�� ���� "+Executions.getCurrent().getUserAgent
	        ...
	        (this.win$cb_branch.getValue() == null) || (this.win$cb_branch.getValue().length() == 0)))
	      {
			...
			if (currusr.getId() > 0)
			{
				...
				if (ipAddress == null)
				{
				 ...
				 //UserService.UsrLog(new UserActionLog(currusr.getId(),win$tb_username.getText(),ipAddress,1, Long.valueOf(1), "������� ���� � �������", currusr.getBranch()));
                 //UserService.UsrLog(new UserActionLog(currusr.getBranch(),uid,win$tb_username.getText(),ipAddress,1),currusr.getAlias());
				//UserService.UsrLog(new UserActionsLog(currusr.getId(), win$tb_username.getText(), ipAddress, 1, Long.valueOf(1), "������� ���� � �������", currusr.getBranch()));
	...
	private void addModule(int userId, String lang)
	{
		...
		try
		{
			...
			mmdl = new Menupopup();
			mmdl_2 = new Menupopup();
			mmdl.setId("mmdl");
			mmdl_2.setId("mmdl_2");
			m.appendChild(mmdl);
			m2.appendChild(mmdl_2);
			menubar.appendChild(m);
