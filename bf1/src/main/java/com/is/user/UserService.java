...
public class UserService {
	...
	public static Res chPwdToUser(String user, String pwd, String npwd, String alias) {
		...
		try {
			...
			e.printStackTrace(); com.is.ISLogger.getLogger().error(com.is.utils.CheckNull.getPstr(e));
			res.setName(e.getMessage());
		} finally {
			ConnectionPool.close(c);
		}
	...
	public static OBJ_Res lg(String login, String password) {
		...
		try {
			...
			ps = c.prepareStatement("select u.*,b.user_name alias from v_bf_bank_users u, ss_dblink_branch b where u.branch=b.branch and upper(u.user_name) = upper(?) ");
			...
			if (rs.next()) {
                ...
                PreparedStatement ps_pwd = c.prepareStatement("Select (case "+
							...
							"       and U.USER_NAME = UPPER(?) and u.id < 90000");
			...
			com.is.ISLogger.getLogger().error(CheckNull.getPstr(ex));
			res1.setCode(-1);
		} finally {
			ConnectionPool.close(c);
		}
